...
Public Class Inicio
 ...
 Private Sub ChecaAviso()
  ...
  Try
   'CadenaSQL = "SELECT COUNT(*) as total FROM Alerta_usuarios WHERE (idUsuarioResp = '" + UsrTPM + "' OR idUsuario = '" + UsrTPM + "' OR otrosUsuarios = '" + UsrTPM + "') AND ConfirmacionLectura IS NULL"
   CadenaSQL = "SELECT COUNT(*) as total FROM Alerta_usuarios WHERE (idUsuarioResp = '" + UsrTPM + "' AND ConfirmacionLectura IS NULL) OR (idUsuario = '" + UsrTPM + "' AND ConfirmacionLecturaIdUsuario IS NULL) OR (otrosUsuarios LIKE '%" + UsrTPM + "%' AND ConfirmacionLecturaOtrosUsuarios IS NULL)"
   ...
   'CadenaSQL = "SELECT * FROM Alerta_usuarios WHERE (idUsuarioResp = '" + UsrTPM + "' AND ConfirmacionLectura IS NULL) OR (idUsuario = '" + UsrTPM + "' AND ConfirmacionLecturaIdUsuario IS NULL) OR (otrosUsuarios LIKE '%" + UsrTPM + "%' AND ConfirmacionLecturaOtrosUsuarios IS NULL)"
   CadenaSQL = "SELECT *, CASE WHEN (idUsuarioResp = '" + UsrTPM + "' AND ConfirmacionLectura IS NULL) THEN 1 
	                              WHEN (idUsuario = '" + UsrTPM + "' AND ConfirmacionLecturaIdUsuario IS NULL) THEN 2
		                             WHEN (otrosUsuarios LIKE '%" + UsrTPM + "%' AND ConfirmacionLecturaOtrosUsuarios IS NULL) THEN 3  
		                        END TipoUsuario
                FROM Alerta_usuarios 
                WHERE (idUsuarioResp = '" + UsrTPM + "' AND ConfirmacionLectura IS NULL) 
                   OR (idUsuario = '" + UsrTPM + "' AND ConfirmacionLecturaIdUsuario IS NULL) 
                   OR (otrosUsuarios LIKE '%" + UsrTPM + "%' AND ConfirmacionLecturaOtrosUsuarios IS NULL)"
   ...
   If conexion_universal.rd_s.Read Then
    ...
    lblAlertaUsr.Tag = rd_s.Item("TipoUsuario")
 ...
 Private Sub btn_enteradoAlerta_Click(sender As Object, e As EventArgs) Handles btn_enteradoAlerta.Click
  ...
  If (lblAlertaUsr.Tag = "1") Then
   CadenaSQL = "UPDATE Alerta_usuarios SET ConfirmacionLectura = GETDATE() WHERE id = " & lbl_IdAlert.Text
  ElseIf (lblAlertaUsr.Tag = "2") Then
   CadenaSQL = "UPDATE Alerta_usuarios SET ConfirmacionLecturaIdUsuario = GETDATE() WHERE id = " & lbl_IdAlert.Text
  ElseIf (lblAlertaUsr.Tag = "3") Then
   CadenaSQL = "UPDATE Alerta_usuarios SET ConfirmacionLecturaOtrosUsuarios = GETDATE() WHERE id = " & lbl_IdAlert.Text
  End If
 ...
 Private Sub ComentariosPedidoDiarioToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ComentariosPedidoDiarioToolStripMenuItem.Click
  Dim comentarios As New comenPedidoDiario()
  comentarios.MdiParent = Me
  comentarios.Show()
 End Sub

 Private Sub AdministrarUbicacionesSurtidoToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles AdministrarUbicacionesSurtidoToolStripMenuItem.Click
  Dim UbiSurtido As New UbicacionesAdministrador()
  UbiSurtido.MdiParent = Me
  UbiSurtido.Show()
 End Sub

 Private Sub PartidasFaltantesTarimaToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles PartidasFaltantesTarimaToolStripMenuItem.Click
  Dim tarimas As New Partidas_Faltantes()
  tarimas.MdiParent = Me
  tarimas.Show()
 End Sub

 Private Sub AdministraciónDePesosToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles AdministraciónDePesosToolStripMenuItem.Click
  Dim pesos As New AdministracionPesos()
  pesos.MdiParent = Me
  pesos.Show()
 End Sub

 Private Sub ExistenciasArtículosRemateToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles ExistenciasArtículosRemateToolStripMenuItem.Click
  Dim EAR As New frmExistenciasArticulosRemate()
  EAR.MdiParent = Me
  EAR.Show()
 End Sub

 Private Sub VentasLineasRemateObsoletosToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles VentasLineasRemateObsoletosToolStripMenuItem.Click
  Dim VLRO As New Ventas_linea_remate_Obsoletos
  VLRO.MdiParent = Me
  VLRO.Show()
 End Sub
