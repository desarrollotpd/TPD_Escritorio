	...
	Private Sub fFormatoGrid(ByRef ds As DataSet)
		Try
			With DgDifCompras
				...
				'Coloco esta otra forma de validar los colores porque con la forma de abajo comentada a veces ponia a todos en rojo y no se localizo el motivo
				For i = 0 To (numfilas - 1) 'Pone a todos en negrita por defaul
					DgDifCompras.Rows(i).Cells(14).Style.ForeColor = Color.Black
					DgDifCompras.Rows(i).Cells(15).Style.ForeColor = Color.Black
				Next

				For Each fila As DataGridViewRow In DgDifCompras.Rows
					If fila.Cells("Diferencia").Value < 0 Then
						fila.Cells("Diferencia").Style.ForeColor = Color.Red
					ElseIf fila.Cells("Diferencia").Value >= 0 Then
						fila.Cells("Diferencia").Style.ForeColor = Color.Black
					End If

					If fila.Cells("Diferencia en Porcentaje").Value < 0 Then
						fila.Cells("Diferencia en Porcentaje").Style.ForeColor = Color.Red
					ElseIf fila.Cells("Diferencia en Porcentaje").Value >= 0 Then
						fila.Cells("Diferencia en Porcentaje").Style.ForeColor = Color.Black
					End If
				Next
...
#End Region
	...
	'Private Sub DgDifCompras_RowPostPaint(sender As Object, e As DataGridViewRowPostPaintEventArgs) Handles DgDifCompras.RowPostPaint
	'	If AlgunCambio = True Then
	'		MsgBox("ok")
	'	End If
	'End Sub
    ...
    Private Sub DgDifCompras_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles DgDifCompras.CellClick
