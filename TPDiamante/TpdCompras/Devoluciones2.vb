...
Public Class Devoluciones2
	...
	Dim SQLConsultaDevolucion As String
	...
	''EVENTO LOAD
	Private Sub Devoluciones2_Load(sender As Object, e As EventArgs) Handles MyBase.Load

		If UsrTPM = "ALMACEN1" Then
			Button1.Location = New Point(650, 45)
			BSave.Location = New Point(675, 95)
			Label1.Location = New Point(650, 122)
		End If
		'DGDevoluciones.Columns.Clear()
	...
	Private Sub DGDevoluciones_CellEndEdit_1(sender As Object, e As DataGridViewCellEventArgs) Handles DGDevoluciones.CellEndEdit
	...
	Private Sub DGDevoluciones_RowPrePaint(sender As Object, e As DataGridViewRowPrePaintEventArgs) Handles DGDevoluciones.RowPrePaint
	...
	Private Sub Button2_Click_2(sender As Object, e As EventArgs) Handles Button2.Click
		...
		'****************************************
		Try
			DGDevoluciones.Columns.Remove("Eliminar")
		Catch ex As Exception

		End Try

		'Dim NumCols As Integer = 0
		'For Each Columna As DataGridViewColumn In DGDevoluciones.Columns
		'	If Columna.Name = "Eliminar" Then
		...
		Consulta &= " SELECT Estado,DIASTRANSTOT,FecSuc,FecAlm,Factura,FecFac,"
		...
		Else
			...
			'Consulta &= " ORDER BY id,FecAlm asc"
		...
		Try
			...
			If (DGDevoluciones.RowCount <= 0) Then
				Return
			End If
		...
		If (NumCols <= 17) Then
			...
			DGDevoluciones.Columns.Insert(17, Boton)
		End If
	...
	Private Sub DGDevoluciones_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles DGDevoluciones.CellContentClick
		Dim rowC As DataGridViewRow = DGDevoluciones.CurrentRow()
		If e.RowIndex >= 0 Then
			If Me.DGDevoluciones.Columns(e.ColumnIndex).Name <> "Eliminar" Then
			Else

				Dim PosicionGrid As Integer

				PosicionGrid = DGDevoluciones.CurrentRow.Index

				Dim FacturaDev As Integer
				Dim Articulo As String

				FacturaDev = DGDevoluciones.Item(4, PosicionGrid).Value
				Articulo = DGDevoluciones.Item(12, PosicionGrid).Value


				If (MessageBox.Show("¿Desea eliminar la devolución del artículo " & Articulo & " de la factura " & FacturaDev & "?",
																															"Confirmacion", MessageBoxButtons.YesNo, MessageBoxIcon.Warning) = Windows.Forms.DialogResult.No) Then
				Else

					SQLConsultaDevolucion = "delete from DetalleFacturaDevolucion WHERE Factura =@FacturaDev "

					conexion_universal.conectar()
					'CONSULTA DE OBTENCIÓN DEL NOMBRE DEL USUARIO
					conexion_universal.slq_s = New SqlCommand(SQLConsultaDevolucion, conexion_universal.conexion_uni)

					conexion_universal.slq_s.Parameters.AddWithValue("@FacturaDev", FacturaDev)
					'EJECUTA LA CONSULTA
					conexion_universal.rd_s = conexion_universal.slq_s.ExecuteScalar
					conexion_universal.cerrar_conectar()

					DGDevoluciones.Rows.Remove(DGDevoluciones.Rows(DGDevoluciones.CurrentRow.Index))
	...
	Private Sub DGDevoluciones_CellDoubleClick(sender As Object, e As DataGridViewCellEventArgs) Handles DGDevoluciones.CellDoubleClick
		If Me.DGDevoluciones.Columns(e.ColumnIndex).Name = "Estatus" Then
			...
			For i = 0 To DGDevoluciones.RowCount - 1
				...
				Try
					...
					cmd4 = New SqlCommand("SPActualizaDetFactDevolucion4", cnn)
					cmd4.CommandType = CommandType.StoredProcedure
