...
Public Class OVtaCSinM
 ...
 Public Sub EnvioMail(RutaArchivo As String)
  Try
   ...
   smtp.Credentials = New NetworkCredential("desarrollo.ti@tractopartesdiamante.com.mx", "D/y7an@_3022erJ")
   smtp.Port = 587
    ...
    For Each row As IXLRow In workSheet.Rows()
     ...
     Else
      ...
      If Articulo <> "" And ASolicitar > 0 And Almacen <> "" And Confirmado > 0 Then
       ...
       Dim QryValida As String
       QryValida = "SELECT T0.DocNum Existe FROM SBO_TPD.dbo.OPOR T0
                        INNER JOIN SBO_TPD.dbo.POR1 T1 ON T0.DocEntry = T1.DocEntry
                        WHERE DocNum = " & OrdenCompra.ToString() & "
                        AND T1.ItemCode = '" & Articulo & "' 
                        AND T1.Quantity = " & Confirmado.ToString()
       If SQL.SiExiste(QryValida) Then
 ...
 Private Sub DGVEncOrdVta_CellContentDoubleClick(sender As Object, e As DataGridViewCellEventArgs) Handles DGVEncOrdVta.CellContentDoubleClick
  ...
  If e.RowIndex >= 0 Then
   ...
   If Me.DGVEncOrdVta.Columns(e.ColumnIndex).Name = "Bloquear_Surtido" And strValue = "SI" Then 'Solo modificare las que tengan SI
    If MsgBox("Esta seguro de que este artículo no será surtido?" & Chr(13) & "Al confirmar que SI la caja del empaque podrá ser cerrada! ", MsgBoxStyle.Question + MsgBoxStyle.YesNo, "Por favor confirme") = vbNo Then
     Exit Sub
    Else
     ...
     CadenaSQL = "UPDATE TPDSolicitudArticulos SET Bloquear_Surtido = ''"
     ...
     If CadenaSQL <> "" Then
      ...
      Try
       ...
       transactions = SqlConnection.BeginTransaction(IsolationLevel.ReadCommitted, "AdminBloqueo")
       ...
       'Ahora quito bloqueo de otras tablas
       'CadenaSQL = "UPDATE Operacion_Detalle SET bloqueo_por_PedidoDiario = 0"
       'CadenaSQL &= " WHERE ItemCode = '" + DGVEncOrdVta.Rows(e.RowIndex).Cells("Filtro").Value.ToString + "' "
       'CadenaSQL &= " AND DocNum = " + DGVEncOrdVta.Rows(e.RowIndex).Cells("ordencompra").Value.ToString() + " "
       'And T0.DocNum IN (SELECT value from STRING_SPLIT(@OrdenVentaRelacionada,','))
       'OrdenVentaRelacionada
