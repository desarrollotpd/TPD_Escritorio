...
Public Class VacacionesConsulta
    ...
    Private Sub Button2_Click(sender As Object, e As EventArgs) Handles Button2.Click
        ...
        Try
            ...
            'Consulta &= " DECLARE"
            'Consulta &= " @DER20223 int = 0"
            'Consulta &= " ,@FechaIngreso date = (SELECT FechaIng FROM Empleados WHERE NumEmp = " + CBNomEmp.SelectedValue.ToString + ")"
            'Consulta &= " If (MONTH(@FechaIngreso) < MONTH(GETDATE()))"
            'Consulta &= "  SET @DER20223 = 6"
            'Consulta &= " ELSE IF (MONTH(@FechaIngreso) = MONTH(GETDATE()))"
            'Consulta &= "  IF (DAY(@FechaIngreso) <= DAY(GETDATE()))"
            'Consulta &= "    SET @DER20223 = 6 "
            ...
            '	Consulta &= "when Antiguedad = 1 then 3 + @DER20223 "
            '	Consulta &= "when Antiguedad = 2 then 4 + @DER20223 "
            '	Consulta &= "when Antiguedad = 3 then 4 + @DER20223 "
            '	Consulta &= "when Antiguedad = 4 then 5 + @DER20223 "
            '	Consulta &= "when Antiguedad >= 5 and Antiguedad <= 9 then 6 + @DER20223 "
            '	Consulta &= "when Antiguedad >= 10 and Antiguedad <= 14 then 7 + @DER20223 "
            '	Consulta &= "when Antiguedad >= 15 and Antiguedad <= 19 then 8 + @DER20223 "
            '	Consulta &= "when Antiguedad >= 20 and Antiguedad <= 24 then 9 + @DER20223 "
            '	Consulta &= "when Antiguedad >= 25 and Antiguedad <= 29 then 10 + @DER20223 "
            '	Consulta &= "when Antiguedad >= 30 and Antiguedad <= 34 then 11 + @DER20223 "
            '	Consulta &= "when Antiguedad >= 35 and Antiguedad <= 39 then 12 + @DER20223 "
            '	Consulta &= "end "
            'Else
            '	Consulta &= "case when Antiguedad = 0 then 0 "
            '	Consulta &= "when Antiguedad = 1 then 6 + @DER20223  "
            '	Consulta &= "when Antiguedad = 2 then 8 + @DER20223 "
            '	Consulta &= "when Antiguedad = 3 then 10 + @DER20223 "
            '	Consulta &= "when Antiguedad = 4 then 12 + @DER20223 "
            '	Consulta &= "when Antiguedad >= 5 and Antiguedad <= 9 then 14 + @DER20223 "
            '	Consulta &= "when Antiguedad >= 10 and Antiguedad <= 14 then 16 + @DER20223 "
            '	Consulta &= "when Antiguedad >= 15 and Antiguedad <= 19 then 18 + @DER20223 "
            '	Consulta &= "when Antiguedad >= 20 and Antiguedad <= 24 then 20 + @DER20223 "
            '	Consulta &= "when Antiguedad >= 25 and Antiguedad <= 29 then 22 + @DER20223 "
            '	Consulta &= "when Antiguedad >= 30 and Antiguedad <= 34 then 24 + @DER20223 "
            '	Consulta &= "when Antiguedad >= 35 and Antiguedad <= 39 then 26 + @DER20223 "
            '	Consulta &= "end "
            'End If
            ...
            '	Consulta &= "when AntiguedadParaPeriodo = 1 then 3 + @DER20223  "
            '	Consulta &= "when AntiguedadParaPeriodo = 2 then 4 + @DER20223  "
            '	Consulta &= "when AntiguedadParaPeriodo = 3 then 4 + @DER20223  "
            '	Consulta &= "when AntiguedadParaPeriodo = 4 then 5 + @DER20223  "
            '	Consulta &= "when AntiguedadParaPeriodo >= 5 and AntiguedadParaPeriodo <= 9 then 6 + @DER20223  "
            '	Consulta &= "when AntiguedadParaPeriodo >= 10 and AntiguedadParaPeriodo <= 14 then 7 + @DER20223  "
            '	Consulta &= "when AntiguedadParaPeriodo >= 15 and AntiguedadParaPeriodo <= 19 then 8 + @DER20223  "
            '	Consulta &= "when AntiguedadParaPeriodo >= 20 and AntiguedadParaPeriodo <= 24 then 9 + @DER20223  "
            '	Consulta &= "when AntiguedadParaPeriodo >= 25 and AntiguedadParaPeriodo <= 29 then 10 + @DER20223  "
            '	Consulta &= "when AntiguedadParaPeriodo >= 30 and AntiguedadParaPeriodo <= 34 then 11 + @DER20223  "
            '	Consulta &= "when AntiguedadParaPeriodo >= 35 and AntiguedadParaPeriodo <= 39 then 12 + @DER20223  "
            '	Consulta &= "end as 'DiasVacaParaPeriodo',  "
            'Else
            '	Consulta &= "case when AntiguedadParaPeriodo = 0 then 0  "
            '	Consulta &= "when AntiguedadParaPeriodo = 1 then 6 + @DER20223  "
            '	Consulta &= "when AntiguedadParaPeriodo = 2 then 8 + @DER20223  "
            '	Consulta &= "when AntiguedadParaPeriodo = 3 then 10 + @DER20223  "
            '	Consulta &= "when AntiguedadParaPeriodo = 4 then 12 + @DER20223  "
            '	Consulta &= "when AntiguedadParaPeriodo >= 5 and AntiguedadParaPeriodo <= 9 then 14 + @DER20223  "
            '	Consulta &= "when AntiguedadParaPeriodo >= 10 and AntiguedadParaPeriodo <= 14 then 16 + @DER20223  "
            '	Consulta &= "when AntiguedadParaPeriodo >= 15 and AntiguedadParaPeriodo <= 19 then 18 + @DER20223  "
            '	Consulta &= "when AntiguedadParaPeriodo >= 20 and AntiguedadParaPeriodo <= 24 then 20 + @DER20223  "
            '	Consulta &= "when AntiguedadParaPeriodo >= 25 and AntiguedadParaPeriodo <= 29 then 22 + @DER20223  "
            '	Consulta &= "when AntiguedadParaPeriodo >= 30 and AntiguedadParaPeriodo <= 34 then 24 + @DER20223  "
            '	Consulta &= "when AntiguedadParaPeriodo >= 35 and AntiguedadParaPeriodo <= 39 then 26 + @DER20223  "
            '	Consulta &= "end as 'DiasVacaParaPeriodo',  "
            'End If
            ...
            'Consulta &= "cast(AntiguedadParaPeriodo as varchar) as 'Antiguedad', cast(CASE WHEN (PeriodoKey < 2023 ) THEN DiasVacaParaPeriodo-@DER20223 ELSE DiasVacaParaPeriodo END as varchar) as 'DiasVacaciones',  "
            'Consulta &= "cast((CASE WHEN (PeriodoKey < 2023 ) THEN DiasVacaParaPeriodo-@DER20223 ELSE DiasVacaParaPeriodo END - DiasXAdelantado) as varchar) as 'DiasDisponibles', '' as 'DiaSolicitado', 0 as 'DiasRestantes', "
            'Consulta &= "cast(DiasTomados as varchar) as 'DiasGozados', cast(CASE WHEN (PeriodoKey < 2023 ) THEN DiasRestantes-@DER20223 ELSE DiasRestantes END as varchar) as 'DiasPendientes', 1 as 'Priori', 999 as 'folio', '' as 'Accion' "
            'Consulta &= "into #pre "
            ...
            '	Consulta &= "case when Priori = 1 then '' else CAST(DiasRestantes as varchar) end as 'Dias Restantes' , "
            '	Consulta &= "DiasGozados as 'Dias Gozados', DiasPendientes as 'Dias Pendientes', PeriodoKey as 'Periodo2', Priori, "
            '	Consulta &= "PeriodoKey as 'Periodo2', Priori, "
            '	Consulta &= "case when DiasPendientes = '' then 'NA' "
            '	Consulta &= "else case when CAST(DiasPendientes AS integer) < 0 then 'Negativo' "
            '	Consulta &= "else 'Positivo' end "
            '	Consulta &= "end as 'Estado', folio, Accion "
            ...
            Dim conexion2 As SqlConnection = Nothing
            Dim AdapMObra As SqlDataAdapter
            Dim DsOrdenes = New DataSet

            Dim DataSetX = New DataSet

            Try
                conexion2 = New SqlConnection(StrTpm) 'ORIGINAL
                cmd = New SqlCommand("SP_CalculoConsultaVacaciones", conexion2)
                cmd.CommandType = CommandType.StoredProcedure
                cmd.Parameters.AddWithValue("@NumEmp", CBNomEmp.SelectedValue)
                cmd.Parameters.AddWithValue("@TipoEmpleado", row_azu.Item("Tipo").ToString)
                cmd.Parameters.AddWithValue("@SoloConsulta", "S")
                conexion2.Open()
                AdapMObra = New SqlDataAdapter
                AdapMObra.SelectCommand = cmd
                AdapMObra.SelectCommand.Connection = conexion2
                ...
                Try
                    ...
                    End If
                Catch ex As Exception
                    MessageBox.Show("El empleado " + CBNomEmp.Text + " no tiene vacaciones asignadas...")
                End Try
                cmd.Connection.Close()
                conexion2.Close()
            Catch ex As Exception
                MessageBox.Show("Error al obtener el calculo de Vacaciones. " + ex.ToString, "Error de Conexion o Consulta", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
                Return
            End Try
    ...
    Private Sub LlenarComboboxEmpleado()
        ...
        '' ConsutaLista = "Select numero_empleado As NumEmp ,apellido_paterno +' '+ apellido_materno +' '+ nombre as NomEmp  from TPDW1.dbo.dat_empleado where estatus = 0  order by apellido_paterno asc"
        'ConsutaLista = " Select  distinct  TPDW1.dbo.dat_empleado.numero_empleado As NumEmp, TPDW1.dbo.dat_empleado.apellido_paterno "
        'ConsutaLista &= " + ' ' + TPDW1.dbo.dat_empleado.apellido_materno + ' ' + TPDW1.dbo.dat_empleado.nombre AS NomEmp, "
        'ConsutaLista &= " isnull(Empleados.Tipo,'') as Tipo "
        'ConsutaLista &= " From TPDW1.dbo.dat_empleado LEFT OUTER Join "
        'ConsutaLista &= "   Empleados On TPDW1.dbo.dat_empleado.numero_empleado = Empleados.NumEmp "
        'ConsutaLista &= " Where (TPDW1.dbo.dat_empleado.estatus = 0) "
        'ConsutaLista &= " Order By NomEmp "
