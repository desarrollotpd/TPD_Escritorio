...
Public Class frmOrdenesVta
 Dim ListaRegs As New List(Of RegistrosOrdenes)
...
#Region "Metodos"
 ...
 Sub LlenarDgvOperacion(USUID As String)
  ...
  Try
   ...
   cmd = New SqlCommand("SP_Operacion_Vtas3", cnn)
   ...
   'Envia el parametro del almacen del usuario

   If (Me.CmbSucursal.SelectedValue = Nothing And (UsrTPM = "MANAGER" Or UsrTPM = "AINVEN")) Then
    Exit Sub
   End If

   SQL.conectarTPM()
   If (UsrTPM = "MANAGER") Or (UsrTPM = "AINVEN") Then
    Select Case Trim(Me.CmbSucursal.SelectedValue.ToString)
   ...
   End If
   cmd.Parameters.AddWithValue("@AlmTPM", CveSucursal) ' 220-03-30

   'cmd.Parameters.AddWithValue("@AlmTPM", AlmTPM) ' 220-03-30
   ...
   'Realiza la comparacion de los registros haber si alguno cambio
   Dim AlgunCambio As Boolean
   If (ListaRegs.Count > 0) Then
    For Each drRow As DataRowView In ResultadoOrden
     AlgunCambio = True
     Dim sEntrega = ""
     If (drRow.Item(2) Is DBNull.Value) Then
      sEntrega = ""
     Else
      sEntrega = drRow.Item(2)
     End If
     For Each item As RegistrosOrdenes In ListaRegs
      If item.OrdenVenta = drRow.Item(0) And item.Entrega = sEntrega Then
       If (item.Status = drRow.Item(9)) Then
        AlgunCambio = False
        Exit For
       Else
        If chkAvisoAudio.Checked = True And AvisosAudibles Then
         funciones.AvisoCambioStatus()
        End If
        If sEntrega = "" Then
         StatusBar.Text = "Ha cambiado el status de la Orden No." & item.OrdenVenta & " de " & item.Status & " a " & drRow.Item(9)
         'MessageBox.Show("Ha cambiado el status de la Orden No." & item.OrdenVenta & " de " & item.Status & " a " & drRow.Item(9), "Cambio de Estado", MessageBoxButtons.OK, MessageBoxIcon.Information)
        Else
         StatusBar.Text = "Ha cambiado el status de la Entrega No." & item.Entrega & " de " & item.Status & " a " & drRow.Item(9)
          'MessageBox.Show("Ha cambiado el status de la Entrega No." & item.Entrega & " de " & item.Status & " a " & drRow.Item(9), "Cambio de Estado", MessageBoxButtons.OK, MessageBoxIcon.Information)
         End If
        End If
       End If
     Next
   ...
   ListaRegs.Clear()
   For Each drRow As DataRowView In ResultadoOrden
    Dim Tmp As New RegistrosOrdenes
    With Tmp
     .OrdenVenta = drRow.Item(0)
     If (drRow.Item(2) Is DBNull.Value) Then
      .Entrega = ""
     Else
      .Entrega = drRow.Item(2)
     End If
     .Status = drRow.Item(9)
    End With
    ListaRegs.Add(Tmp)
 ...
 Sub BuscarNuevasOV()
  ...
  Try
   ...
   If (DsOrdenesVenta.Tables("OVs").Rows.Count > 0) Then
    ...
    Timer3.Enabled = True
    If AvisosAudibles Then
     funciones.AvisoAudio()
    End If
   Else
     btnNuevaOrdenVenta.Visible = False
    Timer3.Enabled = False
 ...
 Sub LlenarDetalleOperacionVta(Articulo As String)
  'Verifica si tiene facturacion parcial
  If Me.dgvOperacionOvTA.Rows(dgvOperacionOvTA.CurrentCell.RowIndex).Cells("Facturado").Value = "Facturado Parcialmente" Then
   StatusTabDetalles.Visible = True
   EntregaFacturaParcial.Text = "Se han facturado " & Me.dgvOperacionOvTA.Rows(dgvOperacionOvTA.CurrentCell.RowIndex).Cells("LineasFacturadas").Value & " partidas de las " & Me.dgvOperacionOvTA.Rows(dgvOperacionOvTA.CurrentCell.RowIndex).Cells("PartidasEntrega").Value & " existentes en la entrega"
  Else
   EntregaFacturaParcial.Text = ""
 ...
 Sub LlenarAutorizaciones()
  ...
  Try
   ...
   SQLOrdenes_aut &= "Case WHEN T0.ProcesStat = 'Y' THEN 'Autorizado' WHEN T0.ProcesStat = 'W' THEN 'Pendiente' WHEN T0.ProcesStat = 'N' THEN 'Rechazado' "
   SQLOrdenes_aut &= "WHEN T0.ProcesStat = 'C' THEN 'Cancelado' WHEN T0.ProcesStat = 'P' THEN 'Creado' WHEN T0.ProcesStat = 'A' THEN 'Autorizando' End As Estatus, T7.Remarks AS tipoAP , "
   SQLOrdenes_aut &= "T1.U_NAME AS SolicitadoPor, (SELECT FORMAT(UpdateDate, 'yyyy-MM-dd') FROM WDD1 WHERE WddCode = T0.WddCode AND UserID=" + Userid + " AND (Status = 'Y' OR Status = 'N')) AS FechaDecision, "

   SQLOrdenes_aut &= " CASE WHEN CONVERT(varchar(50),LEN((SELECT UpdateTime FROM WDD1 WHERE WddCode = T0.WddCode AND UserID=" + Userid + " And (Status = 'Y' OR Status = 'N'))))  = '3'  THEN "
   SQLOrdenes_aut &= "STUFF((SELECT UpdateTime FROM WDD1 WHERE WddCode = T0.WddCode AND UserID=" + Userid + " And (Status = 'Y' OR Status = 'N')),2,0,':') "
   SQLOrdenes_aut &= " WHEN CONVERT(varchar(50),LEN((SELECT UpdateTime FROM WDD1 WHERE WddCode = T0.WddCode AND UserID=" + Userid + " And (Status = 'Y' OR Status = 'N'))))  = '4'  THEN "
   SQLOrdenes_aut &= "STUFF((SELECT UpdateTime FROM WDD1 WHERE WddCode = T0.WddCode AND UserID=" + Userid + " And (Status = 'Y' OR Status = 'N')),3,0,':') "
   SQLOrdenes_aut &= "END AS HoraDecision, "

   SQLOrdenes_aut &= "(SELECT Remarks FROM WDD1 WHERE WddCode = T0.WddCode AND UserID=" + Userid + " And (Status = 'Y' OR Status = 'N')) AS ComentariosCobranza , "
   SQLOrdenes_aut &= "t0.Remarks AS ComentarioVentas, "
   SQLOrdenes_aut &= "Stuff((Select  ', '  + V.U_NAME  FROM WDD1 T INNER JOIN OUSR V ON T.UserID = V.USERID WHERE WddCode =T0.WddCode FOR XML PATH('')),1, 2,'') AS Autorizadores, "
   SQLOrdenes_aut &= "(SELECT V.U_NAME FROM WDD1 T INNER JOIN OUSR V ON T.UserID = V.USERID WHERE WddCode = T0.WddCode AND T.UserID=6 And (Status = 'Y' OR Status = 'N')) AS Autorizo, "
...
#Region "Eventos"
 Private Sub FrmOrdenVta_Load(sender As Object, e As EventArgs) Handles MyBase.Load
  ...
  btnNuevaOrdenVenta.Visible = False
  panelOV.Visible = False
  chkAvisoAudio.Checked = False

  If AlmTPM = "01" Then
   chkAvisoAudio.Visible = False
   AvisosAudibles = False
  Else
   chkAvisoAudio.Visible = True
   AvisosAudibles = True
  End If
  ...
  If (Dtoname = "SIST") And Userid <> "30" Then 'Se incluyo en el codgo al usuario AINVEN como permiso especial
...
#Region "Estilos"
 Sub EstiloDgvOperacion()
  ...
  With Me.dgvOperacionOvTA
   ...
   'PartidasEntrega
   .Columns("PartidasEntrega").Visible = False
   'Facturado
   .Columns("Facturado").Visible = False
   'LineasFacturadas
   .Columns("LineasFacturadas").Visible = False
  End With
 End Sub
 ...
 Sub EstiloDetalleOperacionVta()
  ...
  If UsrTPM <> "COBRANZ2" And UsrTPM <> "COBRANZ4" And UsrTPM <> "NGOMEZ" And UsrTPM <> "COBRANZ4" Then
 ...
 Private Sub dgvOperacionOvTA_CellValueChanged(sender As Object, e As DataGridViewCellEventArgs) Handles dgvOperacionOvTA.CellValueChanged
