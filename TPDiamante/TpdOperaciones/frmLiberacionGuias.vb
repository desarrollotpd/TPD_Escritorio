...
Public Class frmLiberacionGuias
	...
	'			Case "100" : almacenGuias = "01"
	'			Case "102" : almacenGuias = "03"
	'			Case "103" : almacenGuias = "07"
	'		End Select
	'	Else
	'		almacenGuias = SQL.CampoEspecifico("SELECT Almacen FROM Usuarios where Id_Usuario = '" + UsrTPM + "'", "Almacen")
	'	End If
	'	SQL.Cerrar()
	'End Sub
	...
	Sub LlenaLiberacionMat()
		Try
			...
			If almacenGuias = "" Then
				DTVliberacionGuias.Table = SQL.EjecutarProcedimiento("TPD_Liberacion_Guias2", "@Fecha", 1, dtpFecha.Value.ToString("yyyy-MM-dd"))
			Else
				DTVliberacionGuias.Table = SQL.EjecutarProcedimiento("TPD_Liberacion_Guias2", "@Fecha,@CveSucursal", 2, dtpFecha.Value.ToString("yyyy-MM-dd") + "," + almacenGuias)
			End If
	...
	Sub GuiasLiberadas()
		...
		Try
			If dgvLiberadas.Rows.Count <> 0 Then
				...
				dt.Rows.Clear()
			End If
			dgvLiberadas.DataSource = SQL.EjecutarProcedimiento("TPD_Guias_Liberadas2", "@CveSucursal", 1, almacenGuias)
			EstiloLiberadas()
		Catch ex As Exception
	...
	'Private Sub CmbSucursal_Click(sender As Object, e As EventArgs) Handles CmbSucursal.Click
	'	LlenaLiberacionMat()
	'End Sub
End Class