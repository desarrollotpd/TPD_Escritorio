...
Public Class frmLiberacionMatLOG
 ...
 Dim verde As Color = ColorTranslator.FromHtml("#A6FFA0")
 Dim Anaranjado As Color = ColorTranslator.FromHtml("#FFCC80")
 ...
 Private Sub frmLiberacionMatLOG_Load(sender As Object, e As EventArgs) Handles MyBase.Load
  'Seccion para manejo de seleccion de sucursal
  SQL.conectarTPM()
  ...
  If (UsrTPM = "MANAGER" Or UsrTPM = "AINVEN") Then
   ...
   Panel2.Visible = True
  End If
  'Termina seccion para manejo de seleccion de sucursal

  ActualilzaInf(True)
  'SE ASIGNA EL TIEMPO DE RECARGA DEL GRID EN MILISEGUNDOS
  Timer1.Interval = 100000 'Cada 10 segundos
 ...
 Sub LlenaLiberacionMat()
  ...
  Try
   ...
   'Seccion para el manejo de seleccion de sucursal para MANAGER
   ...
   Else
    CveSucursal = SQL.CampoEspecifico("SELECT Almacen FROM Usuarios where Id_Usuario = '" + UsrTPM + "'", "Almacen")
   End If
   cmd1.Parameters.AddWithValue("@CveSucursal", CveSucursal)
   'Termina Seleccion de sucursal
 ...
 Sub MaterialLiberado()
  ...
  Try
   ...
   tbLiberadas = SQL.ConsultarTabla("TPD_Material_Liberado '" & CveSucursal & "'")
 ...
 Private Sub ActualilzaInf(matLibrado As Boolean)
  cargando(True)
  LlenaLiberacionMat()

  If matLibrado = True Then
   MaterialLiberado()
  End If
  cargando(False)
 End Sub
 ...
 Private Sub cargando(CargandoStatus As Boolean)
  If CargandoStatus Then
