...
#Region "Metodos"
  ...
  Try
   If dgvOrdenes.Rows.Count <> 0 Then
    ...
    With Me.dgvOrdenes
     ...
     .Columns("ANEXO").DefaultCellStyle.Font = New Font("Microsoft Sans Serif", 10, FontStyle.Regular)
     ...
     .Columns("ANEXO").Width = 65
     .Columns("ANEXO").ReadOnly = True
     .Columns("ANEXO").Frozen = True
     'ORDEN DE VENTA DE ANEXO
     .Columns("ANEXOORDENDEVENTA").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter
     .Columns("ANEXOORDENDEVENTA").DefaultCellStyle.Font = New Font("Microsoft Sans Serif", 10, FontStyle.Bold)
     .Columns("ANEXOORDENDEVENTA").DefaultCellStyle.ForeColor = Color.Red
     .Columns("ANEXOORDENDEVENTA").HeaderText = "Anexo Orden de Venta"
     ...
     .Columns("ANEXOORDENDEVENTA").Width = 120
 ...
 Sub MLlenaOrdenes()
  ...
  'La linea de abajo se comento ya que ahora seran neceasrio tener 2 parametros que son Fecha y CveSucursal
  'tbBandera = SQL.EjecutarProcedimiento("SP_Full_Surtido", "@fecha", 1, dtpFecha.Value.ToString("yyyy-MM-dd"))

  Try
   If (Me.CmbSucursal.SelectedValue = Nothing And UsrTPM = "MANAGER") Then
    Exit Sub
   End If
   ...
   Dim DsOrderVentas As New DataSet
   'Consultas para llenado de grid's, consultas y comparaciones respecto al objetivo
   Dim conec = New SqlConnection(StrTpm)
   Dim cmd = New SqlCommand("SP_Full_Surtido", conec)
   'Dim cmd = New SqlCommand("TPD_ScoreCard_NuevasLineasHalcon", conec)
   cmd.CommandType = CommandType.StoredProcedure
   cmd.Parameters.Add("@fecha", SqlDbType.Date).Value = dtpFecha.Value.ToString("yyyy-MM-dd")
   cmd.Parameters.Add("@CveSucursal", SqlDbType.VarChar).Value = CveSucursal
   conec.Open()
   ...
   cmd.Connection.Close()
   conec.Close()
   adaptador.Fill(DsOrderVentas)

   DsOrderVentas.Tables(0).TableName = "Ordenes"
   tbBandera = DsOrderVentas.Tables(0)

   SQL.Cerrar()
  Catch ex As Exception
   MessageBox.Show(ex.ToString, "Error al obtener Ordenes de venta a surtir")
  End Try
  ...
  If e.RowIndex >= 0 Then
   ...
   If Me.dgvOrdenes.Columns(e.ColumnIndex).Name = "Accion" Then
    ...
    If (dgvOrdenes.Rows(e.RowIndex).Cells("Accion").Value.ToString = "Surtir") Then
     ...
     '****************************************************** 
     'INICIA Se requiere autorizacion para surtir desde la pantalla
     '****************************************************** 
     If funciones.GetBloqueoDeSurtidoEnPantalla() = False Then
      ...
      End If
     Else
      MsgBox("El surtido esta bloqueado desde la pantalla", MsgBoxStyle.Exclamation, "Surtido inhabilitado")
     End If
     '****************************************************** TERMINAR SE REQUIERE AUTORIZACION
    ElseIf (dgvOrdenes.Rows(e.RowIndex).Cells("Accion").Value.ToString = "Surtiendo") Then
     ...
     'INICIA Se requiere autorizacion para surtir desde la pantalla
     '****************************************************** 
     If funciones.GetBloqueoDeSurtidoEnPantalla() Then
      frmDetalleSurtir.btnGuardar.Visible = False
     End If
     '****************************************************** TERMINAR SE REQUIERE AUTORIZACION
  ...
  Try
   If dgvOrdenes.Rows.Count <> 0 Then
    ...
    For i As Integer = 0 To dgvOrdenes.Rows.Count - 1
     If dgvOrdenes.Rows(i).Cells("ANEXO").Value.ToString = "SI" Then
      dgvOrdenes.Rows(i).DefaultCellStyle.BackColor = Color.GreenYellow
