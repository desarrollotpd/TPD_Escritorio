...
#Region "General"
 ...
 Private Sub frmDetalleSurtir_Load(sender As Object, e As EventArgs) Handles MyBase.Load
  ...
  dgvDetalle.CurrentCell = dgvDetalle.Rows(0).Cells(8)
...
#Region "Metodos"
 ...
 Sub MObtieneEncOrden()
  ...
  SQLEncabezado = "SELECT CardCode, CardName, FORMAT(T0.DocDate, 'yyyy-MM-dd') AS DocDate, "
  SQLEncabezado &= "FORMAT(DocDueDate, 'yyyy-MM-dd') AS DocDueDate, Comments, T0.TrnspCode, "
  'CAMBIAR LINEAS AQUI
  SQLEncabezado &= "IIF((SELECT TrnspName FROM SBO_TPD.dbo.OSHP WHERE TrnspCode = T0.TrnspCode) IS NULL, '', (SELECT TrnspName FROM SBO_TPD.dbo.OSHP WHERE TrnspCode = T0.TrnspCode)) AS TrnspName "
  ...
  SQLEncabezado &= "WHERE DocNum = '" + lblDocNum.Text + "' "
 ...
 Sub MLlenaOrdenesDetalle(ByVal DocNum As Integer)
  ...
  Try 'CAPTURA EL ERROR DE CONSULTA O CONEXION DE LA BASE DE DATOS DEL TPD
   'CONECTA A LA BASE DE DATOS
   conexion_universal.conectar()
   'Almacenna la conculta
   SQLDetalle = "SELECT T0.LineNum, T0.ItemCode, T0.Description, T3.ItmsGrpNam AS Linea, IIF(T2.SWeight1 IS NULL, 0, CAST(T2.SWeight1 AS DECIMAL(19,4))) AS Peso, "
   ...
   SQLDetalle &= "INNER JOIN SBO_TPD.dbo.OITW T4 ON T4.ItemCode COLLATE SQL_Latin1_General_CP850_CI_AS = T0.ItemCode "
   SQLDetalle &= "AND T1.CveSucursal COLLATE Modern_Spanish_CI_AS = T4.WhsCode "
 ...
 Sub MEstiloGridDetalle()
  ...
  With Me.dgvDetalle
   ...
   .Columns("Peso").DefaultCellStyle.Format = "N4"
   'PESO X UNIDAD
   ...
   .Columns("PesoxUnidad").DefaultCellStyle.Format = "N4"
   'STOCK
...
#Region "EVENTOS"
 ...
 Private Sub frmDetalleSurtir_FormClosed(sender As Object, e As FormClosedEventArgs) Handles MyBase.FormClosed
  'SE COMENTO PARA OPTIMIZAR EL FORMULARIO
...
#Region "Botones"
 ...
 Private Sub btnCancelar_Click(sender As Object, e As EventArgs) Handles btnCancelar.Click
  'ACTIVAR EL TIMER PARA QUE SE SIGA ACTUALIZANDO 
 ...
 Private Sub btnDescartar_Click(sender As Object, e As EventArgs) Handles btnDescartar.Click
  '********************************************************************************
  'PEQUEÑO CODIGO PARA REALIZAR VALIDACION DE FIRMA
  '********************************************************************************
  Dim FormaFirma = New frmSolicitaFirma
  FormaFirma.lblmsgfirma.Text = "Para descartar este pedido se requiere la autorización del jefe de almacén"
  FormaFirma.ShowDialog()
  If FormaFirma.StatusFirmaAcceso = False Then Exit Sub
  '********************************************************************************
  '********************************************************************************
  ...
  If Confirma = 6 Then
   ...
   Try
    ...
    'ALMACENA LA CONSULTA
    SQLUpdateOrden = "UPDATE Operacion_Orden SET Status = 'A', Observacion = '" + txtCommentStatus.Text + "', UserId_Surtido = NULL, UserId_Update = '" + SurtidorCode + "', "
 ...
 Private Sub btnGuardar_Click(sender As Object, e As EventArgs) Handles btnGuardar.Click
  ...
  Dim Confirma As Integer
  'MANDA LA CONFIRMACIÓN SI REALMENTE REQUIEREN SURTIR LA ORDEN
  Confirma = MessageBox.Show("Una vez Guardada la orden de Venta no se podrá Modificar." & vbCrLf & vbCrLf & "Realmente desea Continuar ?", "Confirmación", MessageBoxButtons.YesNo, MessageBoxIcon.Question)
  'SI SE ELIJE QUE SI, ENTRA EN LA PRIMERA OPCIÓN.
  If Confirma = 6 Then
   'MANDA A LLAMAR EL METODO DE VALIDAR VALORES DE CANTIDAD
   MValidaCantidades()
   'Actualiza el peso de la orden
   Actualiza_Peso()
   ...
   If (Datos_Ok = True) Then
    MsgBox("La Orden de Venta [ " + lblDocNum.Text + " ] se ha surtido de manera Satisfactoria.", MsgBoxStyle.Information, "Proceso finalizado")
   ...
   Else
    ...
    MsgBox("Sucedio un error al finalizar el proceso de surtido, intente nuevamente el proceso, si el inconveniente persiste, favor de contactar al Área de Sistemas.", MsgBoxStyle.Exclamation)
   End If
  End If
