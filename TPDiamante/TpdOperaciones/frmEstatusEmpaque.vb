...
#Region "Declaracion de Variables"
	...
	'Se agrega la variable para identificar la sucursal
	Dim CveSucursal As String
#End Region
#Region "Metodos"
	...
	Sub LlenarEmpaque()
		statusBarObservacion.Visible = False
		...
		'AGREGAR EL CAMPO DE CLAVE DE SUCURSAL

		If (Me.cmbSucursales.SelectedValue = Nothing And (UsrTPM = "MANAGER" Or UsrTPM = "AINVEN")) Then
			Exit Sub
		End If

		SQL.conectarTPM()
		If (UsrTPM = "MANAGER" Or UsrTPM = "AINVEN") Then
			Select Case Trim(Me.cmbSucursales.SelectedValue.ToString)
				Case "100" : CveSucursal = "01"
				Case "102" : CveSucursal = "03"
				Case "103" : CveSucursal = "07"
			End Select
		Else
		...
		tbBandera = SQL.EjecutarProcedimiento("TPD_Estatus_Empaque2", "@CveSucursal", 1, CveSucursal)
		If tbBandera.Rows.Count <> 0 Then
			dgvEmpaque.DataSource = SQL.EjecutarProcedimiento("TPD_Estatus_Empaque2", "@CveSucursal", 1, CveSucursal)
	...
	Sub LlenarEmpaqueOriginal()
		...
		Try
			...
			cmd = New SqlCommand("TPD_Estatus_Empaque2", cnn)
			'cmd = New SqlCommand("EstatusEmpaque", cnn)
	...
	Function MImprimeFormato(entrega As String) As Boolean
		Try
			''VARIABLE DE CADENA DE SQL
			'Dim SQLOrdenes As String
			''VARIABLES DE CONEXION DE LLENADO
			...
			Dim DocEntry2 As String = SQL.CampoEspecifico("select DocEntry from SBO_TPD.DBO.ODLN WHERE DocNum =" + entrega, "DocEntry")
			If IsNumeric(DocEntry2) Then
				DvDetalle_Estatus.Table = SQL.EjecutarProcedimiento("TPD_OrdenEmpaque", "@DocEntry", 1, DocEntry2.ToString)

				'DvDetalle_Estatus.Table = SQL.EjecutarProcedimiento("TPD_OrdenEmpaque", "@DocEntry", 1, entrega)
...
#Region "Eventos"
	...
	Private Sub dgvEmpaque_KeyDown(sender As Object, e As KeyEventArgs) Handles dgvEmpaque.KeyDown
		...
		If (e.KeyCode = Keys.Enter) Then
			...
			ElseIf (dgvEmpaque.CurrentRow.Cells("Accion").Value.ToString = "Empacando") Then
				...
				'Valido que no haya algun bloqueo por pedido diario
				If BloqueEnEmpaque(DocNumSurtido, DocNumEmpacar) = True Then
					MsgBox("Este pedido esta esperando almenos un artículo de pedido diario que ya debió haber sido confirmado por lo que no podrá ser cerrado el empaque hasta que este llegue", MsgBoxStyle.Information, "En espera de artículo")
					Exit Sub
				End If
	...
	Private Sub CargaSucursales()
		...
		'*********************************************************************************************************************************************
		'Codigo para obtener informacion de las sucursales en el caso de que sea MANAGER
		'*********************************************************************************************************************************************
		...
		If UsrTPM = "MANAGER" Or UsrTPM = "COMERCIAL" Or UsrTPM = "CGARCIA" Or UsrTPM = "AINVEN" Then
			...
			Using SqlConnection As New Data.SqlClient.SqlConnection(StrCon)
				...
				cmbSucursales.DataSource = DSetTablas.Tables("Sucursales")
				cmbSucursales.DisplayMember = "GroupName"
				cmbSucursales.ValueMember = "GroupCode"
...
#Region "Estilo"
	Sub EstiloEmpaque()
		...
		With Me.dgvEmpaque
			...
			.Columns("ANEXO").DefaultCellStyle.Font = New Font("Microsoft Sans Serif", 8.5, FontStyle.Bold)
			.Columns("ANEXO").HeaderText = "Anexo"
			.Columns("ANEXO").HeaderCell.Style.WrapMode = DataGridViewTriState.True
			.Columns("ANEXO").DefaultCellStyle.WrapMode = DataGridViewTriState.True
			.Columns("ANEXO").Width = 50
			.Columns("ANEXO").ReadOnly = True

			'ANEXO ORDEN DE VENTA
			.Columns("ANEXOORDENDEVENTA").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter
			.Columns("ANEXOORDENDEVENTA").DefaultCellStyle.Font = New Font("Microsoft Sans Serif", 8.5, FontStyle.Bold)
			.Columns("ANEXOORDENDEVENTA").HeaderText = "Anexo O.V."
			.Columns("ANEXOORDENDEVENTA").HeaderCell.Style.WrapMode = DataGridViewTriState.True
			.Columns("ANEXOORDENDEVENTA").DefaultCellStyle.WrapMode = DataGridViewTriState.True
			.Columns("ANEXOORDENDEVENTA").Width = 70
			.Columns("ANEXOORDENDEVENTA").ReadOnly = True
			...
			'Aqui agrego el comentario para almacen que solicitaron
			.Columns("Observacion_p_Almacen").DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleCenter
			.Columns("Observacion_p_Almacen").DefaultCellStyle.Font = New Font("Microsoft Sans Serif", 8, FontStyle.Regular)
			.Columns("Observacion_p_Almacen").HeaderText = "Observacion_p_Almacen"
			.Columns("Observacion_p_Almacen").Width = 115
			.Columns("Observacion_p_Almacen").ReadOnly = True
			.Columns("Observacion_p_Almacen").Visible = False
		End With
	End Sub
	...
	Private Sub dgvEmpaque_CellClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgvEmpaque.CellClick
		...
		Try
			...
			'Reviso si tiene algun comentario extra para almacen
			If (row.Cells("Observacion_p_Almacen").Value <> "") Then
				ts_ComentarioAlmacen.Text = row.Cells("Observacion_p_Almacen").Value
				ts_ComentarioAlmacen.Visible = True
				statusBarObservacion.Visible = True
			Else
				ts_ComentarioAlmacen.Text = ""
				ts_ComentarioAlmacen.Visible = False
				statusBarObservacion.Visible = False
			End If
			...
			ElseIf Me.dgvEmpaque.Columns(e.ColumnIndex).Name = "Imprimir" And row.Cells("Accion").Value = "Por Empacar" Then
				...
				If dgvEmpaque.Rows(e.RowIndex).Cells("Observacion_p_Almacen").Value.ToString.Trim <> "" Then
					MsgBox("COMENTARIO PARA EL EMPACADOR..." & Chr(13) & dgvEmpaque.Rows(e.RowIndex).Cells("Observacion_p_Almacen").Value.ToString.Trim, MsgBoxStyle.Information, "Aviso para el empacador")
				End If
		...
		Try
			...
			If dgvEmpaque.Rows.Count <> 0 Then
				...
				For i As Integer = 0 To dgvEmpaque.Rows.Count - 1
					...
					If dgvEmpaque.Rows(i).Cells("ANEXO").Value.ToString = "SI" Then
						dgvEmpaque.Rows(i).DefaultCellStyle.BackColor = Color.GreenYellow
						dgvEmpaque.Rows(i).DefaultCellStyle.ForeColor = Color.Black
					Else
						'COMPARA EL ESTATUS
	...
	Private Sub Button1_Click(sender As Object, e As EventArgs) Handles ReimpresionPL.Click
		...
		Dim NumEntrega As String = InputBox("Indique el número de Orden de Entrega", "Numero de Orden de Entrega")
		If (NumEntrega.Trim = "") Then Exit Sub
		If IsNumeric(NumEntrega) Then
			If MImprimeFormato(NumEntrega) = False Then
				MsgBox("[" & NumEntrega & "] no es un número de Orden de Entrega válido")
			End If
		Else
			MsgBox("[" & NumEntrega & "] no es un número de Orden de Entrega válido")
		End If
	...
	'Private Sub dgvEmpaque_CellMouseMove(sender As Object, e As DataGridViewCellMouseEventArgs) Handles dgvEmpaque.CellMouseMove
	'	If e.ColumnIndex < 0 Or e.RowIndex < 0 Then Exit Sub
	'	Dim ToolT As String = dgvEmpaque.Rows(e.RowIndex).Cells("Observacion_p_Almacen").Value.ToString

	'	If ToolT.Trim <> "" Then
	'		Me.ToolTip1.SetToolTip(dgvEmpaque, ToolT)
	'	Else
	'		Me.ToolTip1.SetToolTip(dgvEmpaque, "")
	'	End If
	'End Sub
