...
Module funciones
    ...
    Public Function GetBloqueoDeSurtidoEnPantalla() As Boolean
        Dim SQL As New Comandos_SQL()
        SQL.conectarTPM()
        Dim Bloqueado As Boolean = SQL.CampoEspecifico("SELECT BloqueoDeSurtidoEnPantalla 
		                                                FROM ConfiguracionGeneralTPD
                                                  WHERE CveSucursal = '" & AlmTPM & "'", "BloqueoDeSurtidoEnPantalla")
        SQL.Cerrar()
        Return Bloqueado
    End Function

    Public Function BloqueEnEmpaque(OrdenVenta As String, NumeroEntrega As String) As Boolean
        Dim SQL As New Comandos_SQL()
        SQL.conectarTPM()
        Dim Bloqueado As Boolean = SQL.CampoEspecifico("SELECT CASE WHEN SUM(bloqueo_por_PedidoDiario) > 0 THEN 1 ELSE 0 END Bloqueado 
		                                                FROM Operacion_Detalle_Entrega T0
		                                                INNER JOIN SBO_TPD.dbo.ODLN T1 ON T0.DocEntry = T1.DocEntry
                                                  WHERE T0.DocNum = " & OrdenVenta & " AND T1.DocNum = " & NumeroEntrega, "Bloqueado")
        SQL.Cerrar()
        Return Bloqueado
    End Function
    ...
    Public Function Actualizar_Operacion_Entrega_Revision(UserAccion As String, UserId As Int16, Status As String,
                                                          Accion As String, Documento As Integer,
                                                          Optional UserAccion2 As String = "",
                                                          Optional UserId2 As Int16 = 0) As Boolean
        Dim QueryUpdate As String
    ...
    Public Function GetCredencialesEmail(IdUsuario As String) As Boolean
        Dim SQL As String
        Dim Res As Boolean = False

        SQL = "SELECT CorreoE, Pswmail FROM Usuarios WHERE Id_Usuario = '" & IdUsuario & "'"
        ...
        Try
            ...
            conexion_universal.rd_s = conexion_universal.slq_s.ExecuteReader()
            If (conexion_universal.rd_s.Read) Then
                vCorreo = rd_s.Item("CorreoE")
                vPswmail = rd_s.Item("Pswmail")
                Res = True
            Else
                Res = False
            End If
    ...
    Public Sub AvisoAudio()
        'My.Computer.Audio.Volume(55)
        My.Computer.Audio.Play(My.Resources.alarma_humo, AudioPlayMode.Background)
    End Sub

    Public Sub AvisoCambioStatus()
        My.Computer.Audio.Play(My.Resources.mision_imposible, AudioPlayMode.Background)
    End Sub
