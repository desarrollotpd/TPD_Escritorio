...
Public Class frmBonoMensualParametros
 ...
 Private Sub LlenaGridObjetivos()
  ...
  With DGImporteBono
   ...
   Try
    ...
    .Columns("MinimoParaBono").Visible = False
    .Columns("Porc_ScoreCard").Visible = False
    .Columns("Porc_Clientes").Visible = False
    .Columns("Porc_Halcon").Visible = False
    .Columns("Porc_LineasObjetivo").Visible = False
 ...
 Private Sub setValoresPorcentajes(AgenteSeleccionado As Int16)
  Dim DRV As DataRowView = DvPorAgentes.Item(AgenteSeleccionado)
  lblAgenteSeleccionado.Text = DRV.Row("slpcode")
  Label1.Text = "Porcentajes del agente: " & DRV.Row("slpname")
  txtMinimoparabono.Text = Trim(DRV.Row("MinimoParaBono").ToString)
  txtScoreCard.Text = Trim(DRV.Row("Porc_ScoreCard").ToString)
  txtClientes.Text = Trim(DRV.Row("Porc_Clientes").ToString)
  txtLineasHalcon.Text = Trim(DRV.Row("Porc_Halcon").ToString)
  txtLineasObjetivo.Text = Trim(DRV.Row("Porc_LineasObjetivo").ToString)
 End Sub
 ...
 Private Function ValidaPorcentajes() As Boolean
  Dim SumaP As Decimal = 0
  SumaP = SumaP + CDbl(txtScoreCard.Text)
  SumaP = SumaP + CDbl(txtClientes.Text)
  SumaP = SumaP + CDbl(txtLineasHalcon.Text)
  SumaP = SumaP + CDbl(txtLineasObjetivo.Text)
  ...
  If SumaP = 100.0 Then
   Return True
  Else
   MessageBox.Show("la suma de los porcentajes debe dar un total del 100", "Por favor valide los porcentajes", MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
   Return False
  End If
 ...
 Private Sub btnGrabaGenerales_Click(sender As Object, e As EventArgs) Handles btnGrabaGenerales.Click
  SQL.conectarTPM()
  Try
   If ValidaPorcentajes() = False Then
    Exit Sub
   End If
   ...
   Dim Existe As String = SQL.CampoEspecifico("SELECT count(*) as Total FROM BonoMensual_ParametrosPorAgente WHERE Año = " & CBAño.Text & " AND Mes = " & CBMES.SelectedIndex + 1, "Total")
   ...
   If Existe > 0 Then
    If MsgBox("Este Periodo ya esta registrado, desea modificarlo?", MsgBoxStyle.Question + MsgBoxStyle.YesNo, "Por favor confirme") = vbNo Then
     Exit Sub
    Else
     SQL.EjecutarComando("UPDATE BonoMensual_ParametrosPorAgente Set MinimoParaBono = " & Decimal.Parse(txtMinimoparabono.Text.ToString) &
                             ...
                             " WHERE Año = " & CBAño.Text &
                             " AND Mes = " & (CBMES.SelectedIndex + 1) &
                             " AND slpcode = '" & lblAgenteSeleccionado.Text & "'")
   ...
   Try
    ...
    If Existe > 0 Then
     ...
     Else
      ...
      SQL.EjecutarComando("UPDATE BonoMensual_ParametrosPorAgente SET ImporteBonoMensual = " & Decimal.Parse(txtCantidadLinea.Text.ToString) &
                          ", MinimoParaBono = " & txtMinimoparabono.Text &
                          ", Porc_ScoreCard= " & txtScoreCard.Text &
                          ", Porc_Clientes= " & txtClientes.Text &
                          ", Porc_Halcon= " & txtLineasHalcon.Text &
                          ", Porc_LineasObjetivo= " & txtLineasObjetivo.Text &
                          " WHERE slpcode = " & CmbAgteVta.SelectedValue & " AND anio = " & CBAño.Text & " AND mes = " & CBMES.SelectedIndex + 1)
      Buscar_Parametros()
     ...
     End If
    Else
     Dim SqlSQry As String = "INSERT INTO BonoMensual_ParametrosPorAgente (slpcode, slpname, mes, anio, ImporteBonoMensual, MinimoParaBono, Porc_ScoreCard, Porc_Clientes, Porc_Halcon, Porc_LineasObjetivo) 
 ...
 Private Sub btnEliminarRegistro_Click(sender As Object, e As EventArgs) Handles btnEliminarRegistro.Click
  Try
   ...
   If (MessageBox.Show(
    ...
    If Previo >= 0 Then
     selectRenglon(Previo)
     'Coloco los porcentajes del renglon seleccionado
     setValoresPorcentajes(Previo)
    End If
 ...
 Private Sub DGImporteBono_CellContentClick(sender As Object, e As DataGridViewCellEventArgs) Handles DGImporteBono.CellContentClick
  ...
  'Coloco los porcentajes del renglon seleccionado
  setValoresPorcentajes(e.RowIndex)
 End Sub
 ...
 Private Sub Button1_Click_1(sender As Object, e As EventArgs) Handles Button1.Click
  ...
  Try
   ...
   SQL.EjecutarComando("INSERT INTO BonoMensual_ParametrosPorAgente (slpcode, slpname, mes, anio, ImporteBonoMensual, MinimoParaBono, Porc_ScoreCard, Porc_Clientes, Porc_Halcon, Porc_LineasObjetivo) SELECT slpcode, slpname, " & MesToPut & ", " & YearToPut & ", ImporteBonoMensual, MinimoParaBono, Porc_ScoreCard, Porc_Clientes, Porc_Halcon, Porc_LineasObjetivo FROM BonoMensual_ParametrosPorAgente WHERE mes = " & MesToFind & " AND Anio = " & YearToFind)
   'Copio el primer paramentro a la tabla de BonoMensual para que pase
   SQL.EjecutarComando("INSERT INTO BonoMensual_ParametrosGenerales (Año, Mes, MinimoParaBono, Porc_ScoreCard, Porc_Clientes, Porc_Halcon, Porc_LineasObjetivo) 
                        SELECT TOP(1) " & YearToPut & "," & MesToPut & ", MinimoParaBono,Porc_ScoreCard, Porc_Clientes, Porc_Halcon, Porc_LineasObjetivo 
                        FROM BonoMensual_ParametrosPorAgente WHERE mes = " & MesToFind & " AND anio = " & YearToFind & " ORDER BY slpname ASC")
 ...
 Private Sub DGImporteBono_MouseUp(sender As Object, e As MouseEventArgs) Handles DGImporteBono.MouseUp
