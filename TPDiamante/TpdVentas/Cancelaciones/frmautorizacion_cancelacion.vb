...
Public Class frmautorizacion_cancelacion
 ...
 End Sub
    ...
    Sub actualiza_factura()
        ...
        Dim HorasTranscurridas As Integer
        Dim Fi As Date
        Dim Ff As Date
        ...
        FechaActual = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss")


        'FechaActual = "2022-09-06 15:23:39.117"
        '-----
        ...
        Try 'CAPTURA EL ERROR QUE SE OBTENGA
            ...
            If cbautorizado.Checked = True Then
                ...
                Try 'CAPTURA EL ERROR
                    ...
                    SQLCadena = "SELECT DISTINCT(T0.DocNum),t0.docdate +' '+ CASE WHEN Len(t0.DocTime)=4 THEN Stuff(t0.DocTime,3,0,':') WHEN LEN(t0.DocTime) =3 THEN Stuff(t0.DocTime,2,0,':') end as  DocDate, T1.WhsCode, T2.WhsName, "
                ...
                HorasTranscurridas = CInt(DateDiff("h", Fi, Ff)) 'OBTIENE DIFERENCIA DE HORAS ENTRE LA FECHA Y HORA ACTUAL Y LA HORA Y FECHA DE EMISIÓN DE LA FACTURA
                'RECCORRE EL TOTAL DE DIAS TRANSCURRIDOS DE LA FECHA DE FACTURA A LA ACTUAL

                'Se cambia este código porque por disposición de nueva facturación sólo se permite la cancelación de facturas en un lapso de 24 horas de ser expedida y por un monto
                ...
                If DocTotal <= 1000 And HorasTranscurridas <= 24 Then
                    Status = "EN PROCESO DE CANCELACION"
                    Auto = "A" 'QUE TIPO DE AUTORIZACIÓN SERA (ALMACEN / CONTABILIDAD)
                ...
                Else
                    If HorasTranscurridas > 24 Then
                        ...
                        If confirma = 6 Then
                            ...
                            Com_Auto = "rebasa las horas después de su expedición"
                        Else
  ...
  Try 'CAPTURA ERROR DE ENVIO DE CORREO
   ...
   If cbautorizado.Checked = True Then
    ...
    Try 'CAPTURA EL ERROR
     ...
     ElseIf Auto = "C" Then
                        ...
                        conexion_universal.slq_s = New SqlCommand("SELECT fase_nombre, correo FROM documents_correos LEFT JOIN Usuarios ON Usuarios.Almacen = Documents_correos.WHSCODE2 where fase_envio = 0 AND Id_Usuario ='" + UsrTPM + "'", conexion_universal.conexion_uni)
                    End If
    ...
    If Auto = "A" Then
     Titulo = "(F2) Solicitud de Cancelación Factura [ " + txtfactura.Text + " ]"
    ElseIf Auto = "C" Then
     Titulo = "(F0) Solicitud de Autorización Factura [ " + txtfactura.Text + " ]"
    End If
   ...
   ElseIf cbno_procede.Checked = True Then
    ...
    Try 'CAPTURA EL ERROR
     ...
     conexion_universal.slq_s = New SqlCommand("SELECT fase_nombre, correo FROM documents_correos LEFT JOIN Usuarios ON Usuarios.Almacen = Documents_correos.WHSCODE2 where fase_envio = 4 AND Id_Usuario ='" + UsrTPM + "'", conexion_universal.conexion_uni)
     'EJECUTA LA CONSULTA
     conexion_universal.rd_s = conexion_universal.slq_s.ExecuteReader()
