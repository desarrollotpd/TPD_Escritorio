...
Public Class NC_Facts
    ...
    Private Sub ProcesaNCR(DrNCFacts As SqlDataReader)
        ...
        SQLTPD &= "T1.DocDate AS FchFact,T1.DocDueDate AS FchVenc, T1.DocTotal AS ImpFact, T0.NCTotal,T1.DocEntry,t2.descripcion as MetodoPagoFac, t3.descripcion as FormaPagoFac "
        SQLTPD &= "INTO #TOT_NCFAC "
        SQLTPD &= "FROM #NC_FAC_R T0 LEFT JOIN OINV T1 ON T0.Factura = T1.DocNum  "
        SQLTPD &= "left join TPDW1.dbo.cat_metodo_pago t2 on t2.metodo_pago COLLATE Modern_Spanish_CI_AI  = t1.U_BXP_FORMAPAGO "
        SQLTPD &= "left join TPDW1.dbo.cat_forma_pago t3 on t3.forma_pago COLLATE Modern_Spanish_CI_AI = t1.U_BXP_METOPAGO "
        ...
        SQLTPD &= "T0.FchFact, T0.FchVenc,T0.ImpFact, T0.NCTotal,T0.MetodoPagoFac,T0.FormaPagoFac, T2.DocTotal, T2.DocNum, T2.DocDate "
        ...
        SQLTPD &= "ROW_NUMBER() OVER(PARTITION BY T0.NCDocSap,T0.Factura ORDER BY T0.NCDocSap,T0.Factura,T2.DocDate DESC) AS OrdPago,case when t2.Series=85  then 'Pago en parcialidades o diferido' else case when t2.Series = 17 then 'Pago en una sola exhibición' end end as 'MetodoPagoPago',t4.descripcion AS 'FormaPagoPago' "
        ...
        'NUEVA 2019
        SQLTPD &= "INNER JOIN ORCT T2 ON T2.DocEntry = T1.DocNum AND T2.Canceled = 'N' LEFT JOIN ECM2 T3 ON T2.DocEntry = T3.SrcObjAbs AND T3.SrcObjType = 24 LEFT JOIN TPDW1.dbo.cat_forma_pago T4 ON t3.U_B1SYS_PmntMethod = T4.forma_pago COLLATE Modern_Spanish_CI_AI "
        ...
        SQLTPD &= "SELECT T0.Tipo,	T0.NCDocSap, T0.FchNC, T0.NCComent, T0.Factura, T0.IdClte, T0.Nombre,"
        SQLTPD &= "T0.FchFact,T0.FchVenc,T0.ImpFact,T0.MetodoPagoFac,t4.MetodoPagoPago,T0.FormaPagoFac,t4.FormaPagoPago, case when (t4.MetodoPagoPago is null or t0.MetodoPagoFac is null) then null else case when t4.MetodoPagoPago COLLATE Modern_Spanish_CI_AI = t0.MetodoPagoFac then 'NO' else 'SI' END end AS Discrepancia,T1.Devolucion,"
