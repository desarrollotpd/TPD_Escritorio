...
Public Class CambioComisiones
 ...
 Private Sub ConsultaFactura(Sig_Ant As String)
  '***********BUSCAR POR*********** 
  '********NUMERO DE FACTURA*******
  '
  If (Not String.IsNullOrEmpty(Sig_Ant) And Not String.IsNullOrEmpty(TBDocNum.Text)) Then
   If (Sig_Ant.Equals("S")) Then 'Siguiente
    TBDocNum.Text = Integer.Parse(TBDocNum.Text) + 1
   ElseIf (Sig_Ant.Equals("A") And Not String.IsNullOrEmpty(TBDocNum.Text)) Then 'Anterior
    TBDocNum.Text = Integer.Parse(TBDocNum.Text) - 1
   End If
  End If

  txtImporteGeneralmenos.Text = ""
...
RepiteConValor:
 ...
 Private Sub DisenoGrid()
  ...
  With Me.DGDetalle
   ...
   Try
    ...
    .Columns(19).HeaderText = "Comisión Original"
    .Columns(19).Width = 80
 ...
 Private Sub txtImporteGeneralmenos_KeyPress(sender As Object, e As KeyPressEventArgs) Handles txtImporteGeneralmenos.KeyPress
  NumerosyDecimal(txtImporteGeneralmenos, e)
 End Sub
 ...
 Private Sub CambioComisiones_KeyDown(sender As Object, e As KeyEventArgs) Handles Me.KeyDown
 ...
 Private Sub txtImporteGeneralmenos_TextChanged(sender As Object, e As EventArgs) Handles txtImporteGeneralmenos.TextChanged
  Dim ImporteGeneralmenos As Double = 0.0
  Dim Comision As Double = 0.00
  Try
   If Double.TryParse(txtImporteGeneralmenos.Text, ImporteGeneralmenos) Then
    If ImporteGeneralmenos > 0 Then
     If DGDetalle.Rows.Count > 0 Then
      For Each Fila As DataGridViewRow In DGDetalle.Rows
       If Not Fila Is Nothing Then
        Double.TryParse(Fila.Cells(19).Value, Comision)
        If Comision > 0 Then
         Fila.Cells("Comision").Value = Comision - ImporteGeneralmenos
        End If
        'Fila.Cells(13).Value = Fila.Cells(13).Value - Double.Parse(txtImporteGeneralmenos.Text)
       End If
      Next
     Else
      GoTo ColocaOriginales
     End If
    Else
     GoTo ColocaOriginales
    End If
   Else
    'txtImporteGeneralmenos.Text = "0"
    GoTo ColocaOriginales
   End If
...
ColocaOriginales:
   ...
   For Each Fila As DataGridViewRow In DGDetalle.Rows
    If Not Fila Is Nothing Then
     Fila.Cells("Comision").Value = Fila.Cells("ComisionOriginal").Value
    End If
   Next
  Catch ex As Exception
 ...
 Private Sub DGDetalle_CellValueChanged(sender As Object, e As DataGridViewCellEventArgs) Handles DGDetalle.CellValueChanged
  Dim Roww As Integer = e.RowIndex
  Dim Coll As Integer = e.ColumnIndex

  If (Roww <= -1 Or Coll <> 13) Then
   Exit Sub
  End If

  Dim Valor As Double = 0

  Double.TryParse(txtImporteGeneralmenos.Text, Valor)

  If (Valor <= 0) Then
   DGDetalle.Rows(Roww).Cells(19).Value = DGDetalle.Rows(Roww).Cells(13).Value
  End If

  'If (String.IsNullOrEmpty(txtImporteGeneralmenos.Text)) Then
  ' DGDetalle.Rows(Roww).Cells(19).Value = DGDetalle.CurrentCell.Value
  'ElseIf (Valor <= 0.0) Then
  ' DGDetalle.Rows(Roww).Cells(19).Value = DGDetalle.CurrentCell.Value
  'End If
 End Sub
End Class
