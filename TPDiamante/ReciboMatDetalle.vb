...
Public Class ReciboMatDetalle
    ...
    Sub cargar_registros()
        ...
        SQLTPD &= "CASE WHEN T0.DocStatus = 'O' THEN 'Abierto' ELSE 'Cerrado' END AS EstatusDoc, T1.Recibo "
        SQLTPD &= "INTO #T_OCREC "
        SQLTPD &= "FROM OPOR T0 "
        SQLTPD &= "LEFT JOIN TPM.dbo.RECMAT T1 ON T0.DocNum = T1.DocNum "
        SQLTPD &= "WHERE T0.CANCELED = 'N'  "
        ...
        SQLTPD &= "T0.FchRec, T0.DiasEnt, T0.DiasTrans, T0.DiasAtraso, T3.WhsName AS Almacen, "
        ...
        SQLTPD &= "AND T0.OrdCompra = @OrdenCompra "


        SQLTPD &= "SELECT T1.LINENUM + 1 AS LineNum, T1.ItemCode AS Articulo,T1.Dscription AS Descripcion, T5.ItmsGrpNam AS Linea, T6.WhsName AS Almacen, "
        ...
        '***Consulta 3
        SQLTPD &= "SELECT LineNum,Articulo,Descripcion,Linea,Almacen,Cantidad,Recibido,Pendiente,Existencia,Precio,  "
        SQLTPD &= "TotalLinea,Estatus,OrdCompra "
        SQLTPD &= "from #T_DETOCREC T0 ORDER BY T0.DOCENTRY,T0.LINENUM "
        ...
        With comando
            .Parameters.AddWithValue("@OrdenCompra", Module1.detalle)
        ...
        DsRecMat.Tables(1).TableName = "DetOrdComp"
        ...
        With Me.DgvOrdCompra
            ...
            If UsrTPM = "MANAGER" Or UsrTPM = "COMPRAS1" Or UsrTPM = "ACOMPRAS" Or UsrTPM = "AINVEN" Or UsrTPM = "MMAZZOCO" Or UsrTPM = "PRUEBAS" Then
                ''DgvOrdCompra
    ...
    Private Sub ChkVerTodasOrdC_CheckedChanged(ByVal sender As System.Object, ByVal e As System.EventArgs)
        VerOrdCompFacturadas()
    End Sub
    Sub TotalOrdCompra()
    ...
    Sub FiltrarDetOrdComp()
        Try
            DvDetOrdComp.RowFilter = "OrdCompra = '" & DgvOrdCompra.Item(0, DgvOrdCompra.CurrentRow.Index).Value.ToString & "' AND Almacen = '" & DgvOrdCompra.Item(11, DgvOrdCompra.CurrentRow.Index).Value.ToString & "'"
        Catch ex As Exception
        End Try
    End Sub
    Sub FiltrarFactEnt()
    ...
    'Private Sub BtnOCompra_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
    ...
    'Private Sub DgvRecibos_CellEndEdit(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs)
    ...
    Private Sub DgvDetOComp_RowPrePaint(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewRowPrePaintEventArgs) Handles DgvDetOComp.RowPrePaint
        ...
        If Not IsDBNull(DgvDetOComp.Rows(e.RowIndex).Cells("Estatus").Value) Then
            ...
            If DgvDetOComp.Rows(e.RowIndex).Cells("Estatus").Value = "C" Then
                ...
                DgvDetOComp.Rows(e.RowIndex).Cells("Almacen").Style.BackColor = Color.Green
                ...
                DgvDetOComp.Rows(e.RowIndex).Cells("Almacen").Style.ForeColor = Color.White
            ...
            Else
                ...
                DgvDetOComp.Rows(e.RowIndex).Cells("Almacen").Style.BackColor = Color.White
    ...
    Private Sub DgvOrdCompra_RowPrePaint(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewRowPrePaintEventArgs) Handles DgvOrdCompra.RowPrePaint
        ...
        If DgvOrdCompra.Rows(e.RowIndex).Cells("Almacen").Value = "PUEBLA" Then
            DgvOrdCompra.Rows(e.RowIndex).Cells("Almacen").Style.BackColor = Color.LightBlue

        ElseIf DgvOrdCompra.Rows(e.RowIndex).Cells("Almacen").Value = "MÉRIDA" Then
            DgvOrdCompra.Rows(e.RowIndex).Cells("Almacen").Style.BackColor = Color.LightCoral

        ElseIf DgvOrdCompra.Rows(e.RowIndex).Cells("Almacen").Value = "TUXTLA GTZ" Then
            DgvOrdCompra.Rows(e.RowIndex).Cells("Almacen").Style.BackColor = Color.LightGreen
        End If
    ...
    Private Sub Button1_Click_1(sender As Object, e As EventArgs) Handles Button1.Click
        Try
            ...
            Dim NCol As Integer = DgvDetOComp.ColumnCount
            Dim NRow As Integer = DgvDetOComp.RowCount

            fFormatoExcel(exLibro, NRow)

            'Aqui recorremos todas las filas, y por cada fila todas las columnas y vamos escribiendo.
            For i As Integer = 1 To NCol
                exLibro.Worksheets("Hoja1").Cells.Item(8, i) = DgvDetOComp.Columns(i - 1).Name.ToString
            Next

            For Fila As Integer = 0 To NRow - 1

                For Col As Integer = 0 To NCol - 1
                    exLibro.Worksheets("Hoja1").Cells.Item(Fila + 9, Col + 1) = DgvDetOComp.Rows(Fila).Cells(Col).Value
            ...
            exLibro.Worksheets("Hoja1").Rows.Item(8).Font.Bold = 1
            exLibro.Worksheets("Hoja1").Rows.Item(8).HorizontalAlignment = 3
            exLibro.Worksheets("Hoja1").Cells.Range("A8:N8").Interior.ColorIndex = 15
            exLibro.Worksheets("Hoja1").Rows.Item(5).WrapText = True
            'exLibro.Worksheets("Hoja1").Columns.AutoFit()
            exLibro.Worksheets("Hoja1").name = "Detalle de Orden de Compra"
    ...
    Private Sub fFormatoExcel(exLibro As Microsoft.Office.Interop.Excel.Workbook, NRow As Integer)
        Try
            ...
            exLibro.Worksheets("Hoja1").Cells.Range("A2:N2").Merge(True)
            exLibro.Worksheets("Hoja1").Cells.Range("A3:N3").Merge(True)
            exLibro.Worksheets("Hoja1").Cells.Range("A4:N4").Merge(True)
            exLibro.Worksheets("Hoja1").Cells.Range("A5:N5").Merge(True)
            exLibro.Worksheets("Hoja1").Cells.Range("A6:N6").Merge(True)


            ''aplicamos un color de fondo ala celda o rango de celdas
            exLibro.Worksheets("Hoja1").Cells.Range("A1:E1").Interior.ColorIndex = 15
            exLibro.Worksheets("Hoja1").Cells.Range("A2:E2").Interior.ColorIndex = 15
            exLibro.Worksheets("Hoja1").Cells.Range("A3:E3").Interior.ColorIndex = 15
            exLibro.Worksheets("Hoja1").Cells.Range("A4:E4").Interior.ColorIndex = 15
            exLibro.Worksheets("Hoja1").Cells.Range("A5:E5").Interior.ColorIndex = 15
            exLibro.Worksheets("Hoja1").Cells.Range("A6:E6").Interior.ColorIndex = 15


            ''Cambiamos orientacion ala hola
            exLibro.Worksheets("Hoja1").Cells.Item(1, 1) = "Detalle de Orden de compra: " & Module1.detalle
            exLibro.Worksheets("Hoja1").Cells.Item(2, 1) = "Proveedor: " & DgvOrdCompra.Item(3, DgvDetOComp.CurrentCell.RowIndex).Value
            exLibro.Worksheets("Hoja1").Cells.Item(3, 1) = "Fecha Documento: " & DgvOrdCompra.Item(1, DgvDetOComp.CurrentCell.RowIndex).Value
            exLibro.Worksheets("Hoja1").Cells.Item(4, 1) = "Fecha Entrega: " & DgvOrdCompra.Item(1, DgvDetOComp.CurrentCell.RowIndex).Value
            exLibro.Worksheets("Hoja1").Cells.Item(5, 1) = "Almacén: " & DgvOrdCompra.Item(11, DgvDetOComp.CurrentCell.RowIndex).Value
            exLibro.Worksheets("Hoja1").Cells.Item(6, 1) = "Total: $" & DgvOrdCompra.Item(4, DgvDetOComp.CurrentCell.RowIndex).Value
            ...
            exLibro.Worksheets("Hoja1").Columns("B").NumberFormat = "@"
