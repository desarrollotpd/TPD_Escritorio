...
Public Class Cotizacion
 ...
 Dim SinDescuento As Boolean = False

 Dim banderaDescuentos As Integer = 0
 'Dim banderaRecuperar As Integer = 0
 Dim YaEstaAcomodado As Boolean = False
 ...
 Private Sub Cotizacion_Load(sender As Object, e As EventArgs) Handles MyBase.Load
  'VALIDA SI DEBE DE MOSTRAR COTIZAR EN DOLARES POR USUARIO

  lblEspera.Visible = False

  If UsrTPM = "MANAGER" Then
   cbDolares.Visible = True
  End If

  If UsrTPM = "ANCAR" Then
   CkBOtro.Visible = False
   CkBOtro.Enabled = False
   ComboBox1.Enabled = False
  End If

  txtnumCot.Enabled = True
  ...
  Using SqlConnection As New Data.SqlClient.SqlConnection(StrCon)
   ...
   If UsrTPM = "ANCAR" Then
    ConsutaLista &= "and CardCode='C-1417'"
   End If
 ...
 Private Sub CargaListaPrecios()
  ...
  CmdArts.Parameters.AddWithValue("@EsCotizacion", "SI")
 ...
 Private Sub DisenoGridVCap()
  With Me.DGVCap
   ...
   .Columns(6).DataPropertyName = "linea"
   ...
   .Columns(7).DataPropertyName = "Moneda"

   'SE AGREGA LISTA DE PRECIOS 
   .Columns(8).HeaderText = "Lista"
   .Columns(8).Width = 52
   .Columns(8).DefaultCellStyle.Format = "###,###,###"
   .Columns(8).DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
   .Columns(8).ReadOnly = False
   .Columns(8).Visible = True
   .Columns(8).DataPropertyName = "lista"

   .Columns(9).HeaderText = "Importe Original."
   .Columns(9).Width = 70
   .Columns(9).DefaultCellStyle.Format = "###,###,##0.00"
   .Columns(9).DefaultCellStyle.Alignment = DataGridViewContentAlignment.MiddleRight
   .Columns(9).DataPropertyName = "ImporteOriginal"
 ...
 Private Sub DisenoGridVArt()
  With Me.DGVArt
   ...
   'SE AGREGA LISTA DE PRECIOS 
   .Columns(11).HeaderText = "Lista"
   .Columns(11).Width = 52
   .Columns(11).Visible = True

   .Columns(5).HeaderText = "$ Precio, LP: " & ComboBox1.Text.ToString()
   .Columns(5).Width = 65
  ...
  If DGVCap.RowCount > 0 Then
   For i As Integer = 0 To DGVCap.RowCount - 1
    If DGVArt.Item(0, DGVArt.CurrentCell.RowIndex).Value = DGVCap.Item(1, i).Value Then
     ...
     If (MessageBox.Show("Esta seguro de agregar nuevamente el articulo ." & vbCrLf & vbCrLf & "",
     ...
     Else
      Try
       Me.DGVCap.Rows.Add(0, DGVArt(0, DGVArt.CurrentRow.Index).Value.ToString(), DGVArt(1, DGVArt.CurrentRow.Index).Value.ToString(),
                                                  ...
                                                  DGVArt(12, DGVArt.CurrentRow.Index).Value.ToString(), DGVArt(11, DGVArt.CurrentRow.Index).Value.ToString(), 0)
       With DGVCap
 ...
 Private Sub AgregaArticuloRecuperar()
  Dim dt As DataTable = DirectCast(DGVCap.DataSource, DataTable)
  Dim row As DataRow = dt.NewRow()
  Dim row2 As DataGridViewRow = DGVArt.CurrentRow
  row.Item(0) = 0
  row.Item(1) = CStr(row2.Cells(0).Value)  'Artículo
  row.Item(2) = CStr(row2.Cells(5).Value)
  row.Item(3) = CStr(row2.Cells(11).Value)
  row.Item(4) = CStr(row2.Cells(1).Value)
  row.Item(5) = 0
  row.Item(6) = 0
  row.Item(7) = CStr(row2.Cells(12).Value)
  dt.Rows.Add(row)
  DGVCap.DataSource = dt
 End Sub

 Sub acomodarColumnas()
  If (YaEstaAcomodado = False) Then
   DGVCap.Columns(0).DisplayIndex = 0
   DGVCap.Columns(1).DisplayIndex = 1
   DGVCap.Columns(3).DisplayIndex = 2
   DGVCap.Columns(8).DisplayIndex = 3
   DGVCap.Columns(9).DisplayIndex = 4
   YaEstaAcomodado = True
  End If
 End Sub
 ...
 Private Sub BtnMenos_Click(sender As Object, e As EventArgs) Handles BtnMenos.Click
  Try
   If Me.DGVCap.Rows.Count = 0 Then Exit Sub
 ...
 Private Sub BtnGuardar_Click(sender As Object, e As EventArgs) Handles BtnGuardar.Click
  ...
  Dim vTotSIva As Decimal = 0
  Dim vTotDoc As Decimal = 0
  Dim CambiosImportes(0) As String
  Dim IndiceCambio As Byte
  ...
  If MessageBox.Show("¿Confirma que desea guardar y enviar esta cotización?", "Confirmación", MessageBoxButtons.OKCancel, MessageBoxIcon.Question) = Windows.Forms.DialogResult.OK Then
   ...
   'VALIDA QUE SI LA COTIZACION ES EN DOLAR
   If (cbDolares.Checked = True) Then
    'DESHABILITA LA EDICION AL CAMPO DE PRECIO
    DGVCap.Columns(3).ReadOnly = False
    DGVCap.Columns(4).ReadOnly = False
    DGVCap.Columns(5).ReadOnly = False
    Try
     For i = 0 To DGVCap.RowCount - 1
      'MODIFICADO POR IVAN GONZALEZ
      Dim LISTAP As Integer = DGVCap.Item(8, i).Value
      If LISTAP > 7 Then
       LISTAP += 2
      End If
      '---------------------------
      ...
      cmd4 = New SqlCommand("ArticuloBoletin4P", cnn)
      ...
      cmd4.Parameters.AddWithValue("@PrecioDolar", DGVCap.Item(3, i).Value)
      cmd4.Parameters.AddWithValue("@DescDolar", DGVCap.Item(4, i).Value)
      'MODIFICADO POR IVAN GONZALEZ
      cmd4.Parameters.AddWithValue("@ListaPrecio", LISTAP)
      'cmd4.Parameters.AddWithValue("@ListaPrecio", ComboBox1.SelectedValue.ToString())
      '------------------------------------
      cmd4.Parameters.AddWithValue("@Consulta", 0)
      ...
      If DTSinVta.Rows.Count > 0 Then
       For Each row As DataRow In DTSinVta.Rows
        ...
        DGVCap("Expand", i).Value = "USD"
        DGVCap("LISTA", i).Value = LISTAP
    ...
    Finally
     ...
     End If
    End Try
   Else 'ELSE DE VLIDA SI ES COTIZACION EN DOLAR
    'DESHABILITA LA EDICION AL CAMPO DE PRECIO
    'COTIZACION EN PESO MEXICANO
    DGVCap.Columns(3).ReadOnly = True
    DGVCap.Columns(4).ReadOnly = True
    DGVCap.Columns(5).ReadOnly = True
    Try
     ...
     For i = 0 To DGVCap.RowCount - 1
      ...
      'MODIFICADO POR IVAN GONZALEZ
      'NUEVO PROCEDIMIENTO PARA EL PRECIO DEL ARTICULO
      cmd4 = New SqlCommand("TPD_Articulo_Boletin", cnn)

      SQL.conectarTPM()
      Dim SucCliente As String = SQL.CampoEspecifico("SELECT GroupCode FROM SBO_TPD.dbo.OCRD WHERE CardCode = '" + CmbCliente.SelectedValue + "'", "GroupCode")

      ''De acuerdo a la solicitud de Salvador se  realizara lo siguiente. Si el cliente es del grupo 102 (Yucatan) y el articulo es 3030-DP entonces
      ''debera cambiarse el precio de ese articulo 3030-DP y ponerle el precio del articulo 3030HD
      ''Entonces...
      'If (SucCliente = 102) Then
      ' cmd4.Parameters.AddWithValue("@Articulo", "3030HD")
      'Else
      ' cmd4.Parameters.AddWithValue("@Articulo", DGVCap.Item(1, DGVCap.CurrentRow.Index).Value)
      'End If

      'cmd4.Parameters.AddWithValue("@Articulo", DGVCap.Item(1, DGVCap.CurrentRow.Index).Value)
      'cmd4.Parameters.AddWithValue("@Piezas", DGVCap.Item(0, DGVCap.CurrentRow.Index).Value)
      cmd4.Parameters.AddWithValue("@Articulo", DGVCap.Item(1, i).Value)
      cmd4.Parameters.AddWithValue("@Piezas", DGVCap.Item(0, i).Value)
      cmd4.Parameters.AddWithValue("@ListaPrecio", LISTAP)
      cmd4.CommandType = CommandType.StoredProcedure
      ...
      ''--------------------------------------------
      'SE COMENTO POR EL NUEVO PROCEDIMIENTO
      'If DTSinVta.Rows.Count > 0 Then
      '    For Each row As DataRow In DTSinVta.Rows
      '        DGVCap(3, i).Value = row("Price1")
      '        DGVCap(4, i).Value = row("Discount")

      '        'MODIFICADO PÓR IVAN GONZALEZ
      ...
      'NUEVO PROCEDIMIENTO IMPLEMENTADO MEJOR RENDIMIENTO 
      If DTSinVta.Rows.Count <> 0 Then
       Dim posicion As Int16 = -1
       For Each row As DataRow In DTSinVta.Rows
        posicion = posicion + 1
        'DGVCap(3, DGVCap.CurrentRow.Index).Value = row("Price")
        'DGVCap(4, posicion).Value = row("Discount")
        If Format(DGVCap(3, posicion).Value, "###,##0.00") <> Format(row("Price"), "###,##0.00") Then 'Precio normal del articulo
         IndiceCambio = UBound(CambiosImportes) + 1
         ReDim Preserve CambiosImportes(IndiceCambio)
         CambiosImportes(IndiceCambio) = "Cambio el precio del artículo " & row("ItemCode") & Chr(13)
        End If
        DGVCap(3, posicion).Value = row("Price")

        Dim PrecioNeto As Decimal = 0
        If row("ItmsGrpCod") = 129 Then
         PrecioNeto = Decimal.Parse(row("Price")) * Decimal.Parse(row("Dolar"))
         'DGVCap(7, DGVCap.CurrentRow.Index).Value = "MXP"
        Else
         'If Decimal.Parse(row("Discount")) <> 0 Then
         If Decimal.Parse(DGVCap(4, posicion).Value) <> row("Discount") Then
          row("Discount") = Decimal.Parse(DGVCap(4, posicion).Value)
          PrecioNeto = Decimal.Parse(row("Price")) - ((Decimal.Parse(row("Price")) * Decimal.Parse(row("Discount"))))
          'row("Price") = PrecioNeto
         Else
          PrecioNeto = Decimal.Parse(row("Price")) - ((Decimal.Parse(row("Price")) * Decimal.Parse(row("Discount"))))
         End If
        End If
        If Format(DGVCap(5, posicion).Value, "###,##0.00") <> Format((row("Price") - (row("Price") * row("Discount"))) * DGVCap(0, posicion).Value, "###,##0.00") Then
         IndiceCambio = UBound(CambiosImportes) + 1
         ReDim Preserve CambiosImportes(IndiceCambio)
         CambiosImportes(IndiceCambio) = "Cambio el importe del artículo " & row("ItemCode") & Chr(13)
         row("Amount") = Format(DGVCap(5, posicion).Value, "###,##0.00")
        End If
        DGVCap(5, posicion).Value = PrecioNeto * DGVCap(0, posicion).Value
        row("Amount") = DGVCap(5, posicion).Value
       Next
      End If
   ...
   'CONVIERTE EL NUMERO EN LETRA
   NumLetra = Numalet.ToCardinal(vTotDoc.ToString)

   If Trim(txtnumCot.Text) = "" Then 'SI ES UNA COTIZACION NUEVA ENTONCES......
    ...
    '*********************************************************************************************************************
    NumOVta += 1
   Else 'SI ES UNA MODIFICACION DE UNA ACTUALIZACION ENTONCES...
    NumOVta = Trim(txtnumCot.Text)
   End If
   ...
   Try
    If Trim(txtnumCot.Text) <> "" Then 'SI ES UNA COTIZACION NUEVA ENTONCES, ELIMINO LA COTIZACION QUE SERA MODIFICADA Y LUEGO LA VUELVO A CREAR
     strcadena = "DELETE FROM Cotizaciones WHERE IdCot = " & NumOVta
     command.CommandText = strcadena
     command.ExecuteNonQuery()

     strcadena = "DELETE FROM CotizacionesDet WHERE IdCot = " & NumOVta
     command.CommandText = strcadena
     command.ExecuteNonQuery()
    End If
    ...
    For Each row As DataGridViewRow In Me.DGVCap.Rows
     ...
     strcadena = "INSERT INTO CotizacionesDet (IdCot,NumLinea,Articulo,DesArt,Precio,Cantidad,DescLin,TotLinea,Linea, Moneda, ListaPrecios) VALUES ("
     strcadena &= NumOVta.ToString
     strcadena &= ","
     ...
     strcadena &= Valcadena
     strcadena &= "', "
     strcadena &= row.Cells(3).Value 'Precio
     strcadena &= ","
     strcadena &= row.Cells(0).Value ' Cantidad
     strcadena &= ",'"
     strcadena &= row.Cells(4).Value 'Descuento
     strcadena &= "',"
     strcadena &= row.Cells(5).Value 'TotalLinea
     strcadena &= ",'"
     strcadena &= row.Cells(6).Value 'Linea
     strcadena &= "','"
     strcadena &= row.Cells(7).Value 'Moneda
     strcadena &= "',"
     strcadena &= row.Cells(8).Value 'ListaPreciosa
     strcadena &= ")"
   ...
   Catch exc As Exception
    ...
    MessageBox.Show("NO FUE POSIBLE CREAR COTIZACION," & Chr(13) & "POR FAVOR INTENTELO DE NUEVO..." & Chr(13) & exc.Message.ToString, "E R R O R ! ! !",
                     MessageBoxButtons.OK, MessageBoxIcon.Error)
    VErrOv = 1
   ...
   DtOVta.Columns.Add("NumCotizacion", Type.GetType("System.Int32"))
   ...
   DtOVta.Columns.Add("NumLetra", Type.GetType("System.String")) 'COLUMNA DE CONVERSION DE NUMERO A LETRA
   ...
   For Each row As DataGridViewRow In Me.DGVCap.Rows
    ...
    _filaTemp(columnas(1)) = NumOVta.ToString
    _filaTemp(columnas(2)) = Date.Now.ToString
    _filaTemp(columnas(3)) = NomUsuario

    If CmbCliente.Text <> "" Then
     _filaTemp(columnas(4)) = DvClte(CmbCliente.SelectedIndex).Item("Cliente").ToString
     _filaTemp(columnas(5)) = DvClte(CmbCliente.SelectedIndex).Item("Nombre").ToString

    Else
     _filaTemp(columnas(4)) = "Cliente: "
     _filaTemp(columnas(5)) = TBOtro.Text
    End If

    _filaTemp(columnas(6)) = contador
    _filaTemp(columnas(7)) = row.Cells(1).Value  'ARTICULO
    _filaTemp(columnas(8)) = row.Cells(2).Value  'DESCRIPCION
    _filaTemp(columnas(9)) = row.Cells(6).Value  'LINEA
    _filaTemp(columnas(10)) = "1"
    _filaTemp(columnas(11)) = row.Cells(3).Value
    _filaTemp(columnas(12)) = row.Cells(0).Value    'CANTIDAD
    _filaTemp(columnas(13)) = row.Cells(4).Value * 100 'Solo para mostrar el descuento 
    _filaTemp(columnas(14)) = System.Convert.ToDecimal(row.Cells(5).Value)    'TotLinea
    _filaTemp(columnas(15)) = TxtComentario.Text.ToString
    _filaTemp(columnas(16)) = vTotSIva
    _filaTemp(columnas(17)) = vTotIva
    _filaTemp(columnas(18)) = vTotDoc
    'SE AGREGAN CAMPOS AL REPORTE CON EL DATASET: URIEL TORALVA 19/05/2018
    _filaTemp(columnas(19)) = txttipo_cambio.Text.ToString     'TIPO DEL CAMBIO AL DIA
    _filaTemp(columnas(20)) = row.Cells(7).Value               'MONEDA USD O MXP
    _filaTemp(columnas(21)) = NumLetra    'NUMERO ESCRITO CON LETRA
   ...
   'VALIDA SI LA COTIZACION SE MANDA EN DOLAR O EN PESOS NORMALES
   If cbDolares.Checked = True Then
    Dim informe2 As New CrCotizacionDolar
    ...
    'MANDA CORREO
    ParMail()
    Dim informe As New CrCotizacionDolar
    RepComsultaP.MdiParent = Inicio
    ...
    If TxtCorreoAd.Text <> "" And VErrOv = 0 Then
     Dim informe3 As New CrCotizacionDolar
     RepComsultaP.MdiParent = Inicio
     ...
     ParMail3()
    End If
   Else 'ELSE VALIDA SI SE MANDA EN DOLAR O EN PESOS
    ...
    If TxtCorreoAd.Text <> "" And VErrOv = 0 Then
     ...
     ParMail3()
    End If
   End If 'FIN VALIDA SI SE MANDA EN DOLAR O EN PESOS
   ...
   If VErrCAd = 1 Then
    MessageBox.Show("No fue posible enviar la cotizacion al EMail capturado," & Chr(13) & "Verifique el correo electrónico e intente nuevamente..." & Chr(13), "Advertencia",
                      MessageBoxButtons.OK, MessageBoxIcon.Exclamation)
    BtnEmail.Enabled = True
    Return
   Else
 ...
 Sub ParMail()
  ...
  Dim CCO() As String = {vCorreoVta & "," & "tpd@tractopartesdiamante.com.mx"}
  Dim CC() As String = {""}
 ...
 Sub ParMail2()
  ...
  Dim PARA() As String = {TxtCorreoC.Text}
 ...
 Sub ParMail3()
  ...
  Dim PARA() As String = {TxtCorreoAd.Text}
 ...
 Public Sub EnviarMail(ByVal De As String, ByVal Para As String(), ByVal Asunto As String, ByVal Cuerpo As String, Optional ByVal CC As String() = Nothing, Optional ByVal CCO As String() = Nothing)
  ...
  Try
   ...
   Else
    Titulo = "Cotizacion - " & NumOVta.ToString & " - Cliente: " & TBOtro.Text
   End If
   ...
   Dim vMensaje As String = "<html><head></head><body><h2>Cotización </h2>"
  ...
  Catch exc As Exception
   ...
   MessageBox.Show("NO FUE POSIBLE ENVIAR EMAIL DE LA COTIZACION," & Chr(13) & "INTENTE ENVIAR EMAIL NUEVAMENTE..." & Chr(13) & exc.ToString, "E R R O R ! ! !",
             MessageBoxButtons.OK, MessageBoxIcon.Error)
   VErrOv = 1
  Finally
 ...
 Private Sub BtnImprimir_Click(sender As Object, e As EventArgs) Handles BtnImprimir.Click
  ...
  If cbDolares.Checked = True Then
   Try
    ...
    For Each row As DataGridViewRow In Me.DGVCap.Rows
     ...
     _filaTemp = DtOVta.NewRow()
     If NumOVta > 0 Then
      _filaTemp(columnas(0)) = NumOVta
     Else
      If txtnumCot.Text <> "" Then
       _filaTemp(columnas(0)) = Int32.Parse(txtnumCot.Text)
      Else
       _filaTemp(columnas(0)) = 0
      End If
     End If
     _filaTemp(columnas(1)) = NumOVta.ToString
     _filaTemp(columnas(2)) = NumOVta.ToString
     _filaTemp(columnas(3)) = Date.Now.ToString
     _filaTemp(columnas(4)) = NomUsuario

     If CmbCliente.Text <> "" Then
      _filaTemp(columnas(5)) = DvClte(CmbCliente.SelectedIndex).Item("Cliente").ToString
      _filaTemp(columnas(6)) = DvClte(CmbCliente.SelectedIndex).Item("Nombre").ToString
     Else
      _filaTemp(columnas(5)) = "Cliente: "
      _filaTemp(columnas(6)) = TBOtro.Text
     End If

     _filaTemp(columnas(7)) = contador               'NUMERO DE PARTIDAS
     _filaTemp(columnas(8)) = row.Cells(1).Value     'ARTICULO
     _filaTemp(columnas(9)) = row.Cells(2).Value     'DESCRIPCION
     _filaTemp(columnas(10)) = row.Cells(6).Value     'LINEA
     _filaTemp(columnas(11)) = "1"
     _filaTemp(columnas(12)) = row.Cells(3).Value    'PRECIO
     _filaTemp(columnas(13)) = row.Cells(0).Value    'CANTIDAD
     _filaTemp(columnas(14)) = row.Cells(4).Value * 100    'DESCUENTO %
     _filaTemp(columnas(15)) = System.Convert.ToDecimal(row.Cells(5).Value)    'TotLinea
     _filaTemp(columnas(16)) = TxtComentario.Text.ToString
     _filaTemp(columnas(17)) = vTotSIva
     _filaTemp(columnas(18)) = vTotIva
     _filaTemp(columnas(19)) = vTotDoc
     'SE AGREGAN CAMPOS AL REPORTE CON EL DATASET: URIEL TORALVA 19/05/2018
     _filaTemp(columnas(20)) = vTipoCambio           'TIPO DEL CAMBIO AL DIA
     _filaTemp(columnas(21)) = row.Cells(7).Value    'MONEDA USD O MXP
     _filaTemp(columnas(22)) = NumLetra    'NUMERO ESCRITO CON LETRA
  ...
  Else
   Try
    ...
    For Each row As DataGridViewRow In Me.DGVCap.Rows
     ...
     _filaTemp(columnas(6)) = contador               'NUMERO DE PARTIDAS
     _filaTemp(columnas(7)) = row.Cells(1).Value     'ARTICULO
     _filaTemp(columnas(8)) = row.Cells(2).Value     'DESCRIPCION
     _filaTemp(columnas(9)) = row.Cells(6).Value     'LINEA
     _filaTemp(columnas(10)) = "1"
     _filaTemp(columnas(11)) = row.Cells(3).Value    'PRECIO
     _filaTemp(columnas(12)) = row.Cells(0).Value    'CANTIDAD
     _filaTemp(columnas(13)) = row.Cells(4).Value * 100    'DESCUENTO %
     _filaTemp(columnas(14)) = System.Convert.ToDecimal(row.Cells(5).Value)    'TotLinea
     _filaTemp(columnas(15)) = TxtComentario.Text.ToString
     _filaTemp(columnas(16)) = vTotSIva
     _filaTemp(columnas(17)) = vTotIva
     _filaTemp(columnas(18)) = vTotDoc
     'SE AGREGAN CAMPOS AL REPORTE CON EL DATASET: URIEL TORALVA 19/05/2018
     _filaTemp(columnas(19)) = vTipoCambio           'TIPO DEL CAMBIO AL DIA
     _filaTemp(columnas(20)) = row.Cells(7).Value    'MONEDA USD O MXP
     _filaTemp(columnas(21)) = NumLetra    'NUMERO ESCRITO CON LETRA
  ...
  ' For Each row As DataGridViewRow In Me.DGVCap.Rows
  '  Fila += 1
  '  vTotSIva += row.Cells(8).Value
  '  If row.Cells(8).Value = 0 Then      'si la cantidad en algun renglon es 0 marca error "ARTICULO SIN CANTIDAD"
  '   vSinValor = 1
  '   Exit For
  '  End If
  ...
  '    _filaTemp(columnas(20)) = NumLetra    'NUMERO ESCRITO CON LETRA
  '   Else

  '    'MessageBox.Show(row.Cells(0).Value) 'Vacio
  '    'MessageBox.Show(row.Cells(1).Value) 'cantidad
  '    'MessageBox.Show(row.Cells(2).Value) 'articulo
  '    'MessageBox.Show(row.Cells(3).Value) 'precio
  '    'MessageBox.Show(row.Cells(4).Value) 'lista
  '    'MessageBox.Show(row.Cells(5).Value) 'descripcion
  '    'MessageBox.Show(row.Cells(6).Value) 'desc
  '    'MessageBox.Show(row.Cells(7).Value) 'importe
  '    'MessageBox.Show(row.Cells(8).Value) 'MND
  '    'MessageBox.Show(row.Cells(9).Value) 'DP
  '    'MessageBox.Show(row.Cells(10).Value)

  '    _filaTemp(columnas(5)) = contador               'NUMERO DE PARTIDAS
  '    _filaTemp(columnas(6)) = row.Cells(2).Value     'row.Cells(3).Value     'ARTICULO
  '    _filaTemp(columnas(7)) = row.Cells(5).Value     'row.Cells(6).Value     'DESCRIPCION
  '    _filaTemp(columnas(8)) = row.Cells(4).Value     'row.Cells(10).Value     'LINEA
  '    _filaTemp(columnas(9)) = "1"
  '    _filaTemp(columnas(10)) = row.Cells(3).Value    'row.Cells(4).Value    'PRECIO
  '    _filaTemp(columnas(11)) = row.Cells(1).Value    'row.Cells(2).Value    'CANTIDAD
  '    _filaTemp(columnas(12)) = row.Cells(6).Value    'row.Cells(7).Value'DESCUENTO %
  '    _filaTemp(columnas(13)) = System.Convert.ToDecimal(row.Cells(7).Value)    'TotLinea
  '    _filaTemp(columnas(14)) = TxtComentario.Text.ToString
  '    _filaTemp(columnas(15)) = vTotSIva
  '    _filaTemp(columnas(16)) = vTotIva
  '    _filaTemp(columnas(17)) = vTotDoc
  '    'SE AGREGAN CAMPOS AL REPORTE CON EL DATASET: URIEL TORALVA 19/05/2018
  '    _filaTemp(columnas(18)) = vTipoCambio           'TIPO DEL CAMBIO AL DIA
  '    _filaTemp(columnas(19)) = row.Cells(8).Value    'MONEDA USD O MXP
  '    _filaTemp(columnas(20)) = NumLetra    'NUMERO ESCRITO CON LETRA
  '   End If
  ...
  '    _filaTemp(columnas(5)) = contador               'NUMERO DE PARTIDAS
  '    _filaTemp(columnas(6)) = row.Cells(3).Value     'row.Cells(3).Value     'ARTICULO
  '    _filaTemp(columnas(7)) = row.Cells(6).Value     'row.Cells(6).Value     'DESCRIPCION
  '    _filaTemp(columnas(8)) = row.Cells(10).Value     'row.Cells(10).Value     'LINEA
  '    _filaTemp(columnas(9)) = "1"
  '    _filaTemp(columnas(10)) = row.Cells(4).Value    'row.Cells(4).Value    'PRECIO
  '    _filaTemp(columnas(11)) = row.Cells(2).Value    'row.Cells(2).Value    'CANTIDAD
  '    _filaTemp(columnas(12)) = row.Cells(7).Value    'row.Cells(7).Value'DESCUENTO %
  '    _filaTemp(columnas(13)) = System.Convert.ToDecimal(row.Cells(8).Value)    'TotLinea
  '    _filaTemp(columnas(14)) = TxtComentario.Text.ToString
  '    _filaTemp(columnas(15)) = vTotSIva
  '    _filaTemp(columnas(16)) = vTotIva
  '    _filaTemp(columnas(17)) = vTotDoc
  '    'SE AGREGAN CAMPOS AL REPORTE CON EL DATASET: URIEL TORALVA 19/05/2018
  '    _filaTemp(columnas(18)) = vTipoCambio           'TIPO DEL CAMBIO AL DIA
  '    _filaTemp(columnas(19)) = row.Cells(9).Value    'MONEDA USD O MXP
  '    _filaTemp(columnas(20)) = NumLetra    'NUMERO ESCRITO CON LETRA
  '   End If
 ...
 Private Sub BtnEmail_Click(sender As Object, e As EventArgs) Handles BtnEmail.Click
  ...
  Try
   ...
   For Each row As DataGridViewRow In Me.DGVCap.Rows
    ...
    _filaTemp(columnas(11)) = row.Cells(0).Value    'CANTIDAD
    _filaTemp(columnas(12)) = row.Cells(4).Value * 100 'Descuento
    _filaTemp(columnas(13)) = System.Convert.ToDecimal(row.Cells(5).Value)    'TotLinea
    _filaTemp(columnas(14)) = TxtComentario.Text.ToString
    _filaTemp(columnas(15)) = vTotSIva
    _filaTemp(columnas(16)) = vTotIva
 ...
 Private Sub ComboBox1_SelectionChangeCommitted(sender As Object, e As EventArgs) Handles ComboBox1.SelectionChangeCommitted
  ...
  If (Integer.Parse(ComboBox1.SelectedValue.ToString()) <> valor_anterior) Then
   'MsgBox("Cambio lista")
   If (DGVCap.RowCount <> 0) Then
    If (MessageBox.Show("Al cambiar de lista se eliminaran todos los articulos que se han cotizado. ¿Confirma que desea cambiar de lista?",
 ...
 Private Sub cbDolares_CheckedChanged(sender As Object, e As EventArgs) Handles cbDolares.CheckedChanged

 End Sub

 Private Sub DGVCap_CellValueChanged(sender As Object, e As DataGridViewCellEventArgs) Handles DGVCap.CellValueChanged
  ValidoRenglon(e.ColumnIndex)
 End Sub

 Private Sub ValidoRenglon(NumCol As Integer)
  Dim DTSinVta As New DataTable
  Dim cnn As SqlConnection = Nothing
  Dim cmd4 As SqlCommand = Nothing
  Dim row As DataGridViewRow = DGVCap.CurrentRow

  If NumCol <> 0 And NumCol <> 8 Then
   Exit Sub
  End If

  If IsNothing(CmbCliente.SelectedValue) And DGVCap.Rows.Count > 0 Then
   MsgBox("Por favor primero seleccione al cliente", MsgBoxStyle.Critical, "Falta indicar cliente")
   Exit Sub
  End If
  ...
  If DGVCap.Rows.Count > 0 Then
   ...
   lISTA = CStr(row.Cells(8).Value)
   If lISTA = 1 Or lISTA = 2 Or lISTA = 3 Or lISTA = 10 Then
    'MODIFICADO POR IVAN GONZALEZ
    'CORRECCION DE LISTA PRECIO 10
    If lISTA > 7 Then
     lISTA += 2
    End If
    '----------------------------
   Else
    MsgBox("Solo esta permitido cotizar con lista 1,2,3,10 ")
    row.Cells(8).Value = 1
   End If

   If cbDolares.Checked = True Then
    'COTIZACION EN DOLARES
    If NumCol = 0 Or NumCol = 8 Then
     'DESHABILITA LA EDICION AL CAMPO DE PRECIO
     DGVCap.Columns(3).ReadOnly = False
     DGVCap.Columns(4).ReadOnly = False
     DGVCap.Columns(5).ReadOnly = False
     Try
      ...
      cmd4.Parameters.AddWithValue("@PrecioDolar", DGVCap.Item(3, DGVCap.CurrentRow.Index).Value)
      cmd4.Parameters.AddWithValue("@DescDolar", DGVCap.Item(4, DGVCap.CurrentRow.Index).Value)
      'MODIFICADO POR IVAN GONZALEZ
      'CORRECCION DE LISTA PRECIO
      cmd4.Parameters.AddWithValue("@ListaPrecio", lISTA)
      'cmd4.Parameters.AddWithValue("@ListaPrecio", DGVCap.Item(8, DGVCap.CurrentRow.Index).Value)
      '---------------------------------
      cmd4.Parameters.AddWithValue("@Consulta", 0)
      cnn.Open()
      Dim da As New SqlDataAdapter
      ...
      If DTSinVta.Rows.Count > 0 Then
       ...
       For Each fila As DataRow In DTSinVta.Rows
        ...
        DGVCap(9, DGVCap.CurrentRow.Index).Value = "USD"
       Next
       'Else
       'DGVCap(5, DGVCap.CurrentRow.Index).Value = DGVCap(6, DGVCap.CurrentRow.Index).Value * DGVCap.CurrentRow.Cells(0).Value
      End If
     ...
     TxtArticulo.SelectAll()
    Else
     DGVCap(5, DGVCap.CurrentRow.Index).Value = (DGVCap(0, DGVCap.CurrentRow.Index).Value * DGVCap(3, DGVCap.CurrentRow.Index).Value)
    End If
   Else
    'COTIZACION EN PESO MEXICANO
    If DGVCap.Rows.Count > 0 Then
     ...
     If NumCol = 0 Or NumCol = 8 Then
      ...
      Try
       ...
       SQL.conectarTPM()
       Dim SucCliente As String
       SucCliente = SQL.CampoEspecifico("SELECT GroupCode FROM SBO_TPD.dbo.OCRD WHERE CardCode = '" + CmbCliente.SelectedValue + "'", "GroupCode")

       cmd4.Parameters.AddWithValue("@Articulo", DGVCap.Item(1, DGVCap.CurrentRow.Index).Value)
       cmd4.Parameters.AddWithValue("@Piezas", DGVCap.Item(0, DGVCap.CurrentRow.Index).Value)
       cmd4.Parameters.AddWithValue("@ListaPrecio", lISTA)
       cmd4.Parameters.AddWithValue("@SucCte", SucCliente)
       cmd4.CommandType = CommandType.StoredProcedure
       ...
       If DTSinVta.Rows.Count <> 0 Then
        For Each fila As DataRow In DTSinVta.Rows
         ...
         DGVCap(3, DGVCap.CurrentRow.Index).Value = fila("Price")
         'DGVCap(4, DGVCap.CurrentRow.Index).Value = fila("Discount")
         DGVCap(4, DGVCap.CurrentRow.Index).Value = fila("Discount") / 100

         Dim PrecioNeto As Decimal = 0
         If fila("ItmsGrpCod") = 129 Then
          PrecioNeto = Decimal.Parse(fila("Price")) * Decimal.Parse(fila("Dolar"))
          'DGVCap(7, DGVCap.CurrentRow.Index).Value = "MXP"
         Else
          If Decimal.Parse(fila("Discount")) <> 0 Then
           PrecioNeto = Decimal.Parse(fila("Price")) - ((Decimal.Parse(fila("Price")) * Decimal.Parse(fila("Discount"))) / 100)
          Else
           PrecioNeto = Decimal.Parse(fila("Price"))
          End If
         End If
         DGVCap(5, DGVCap.CurrentRow.Index).Value = PrecioNeto * DGVCap.CurrentRow.Cells(0).Value 'Actualizo el importe
         DGVCap(9, DGVCap.CurrentRow.Index).Value = PrecioNeto * DGVCap.CurrentRow.Cells(0).Value 'Actualizo el importe original tambien
        Next
        GoTo CalculaDescuentos
        Exit Sub
       End If
     ...
     Else
      If NumCol <> 5 Then
       DGVCap(5, DGVCap.CurrentRow.Index).Value = (DGVCap(0, DGVCap.CurrentRow.Index).Value * DGVCap(3, DGVCap.CurrentRow.Index).Value)
       DGVCap(9, DGVCap.CurrentRow.Index).Value = (DGVCap(0, DGVCap.CurrentRow.Index).Value * DGVCap(3, DGVCap.CurrentRow.Index).Value)
       GoTo CalculaDescuentos
      End If
     End If
    End If
...
CalculaDescuentos:
 ...
 Private Sub CalculaDscuentos()
  banderaDescuentos = 1

  Using SqlConnection As New Data.SqlClient.SqlConnection(StrTpm)
   conexion.Open()
   Dim comandoD As SqlCommand
   Dim cadena As String = "DELETE FROM tpm.dbo.temporalDesc WHERE idUsuario = '" & UsrTPM & "'"
   comandoD = New SqlCommand(cadena, conexion)
 ...
 Private Sub IngresarTemporales()
  'acomodarColumnas2()
  ...
  Using SqlConnection As New Data.SqlClient.SqlConnection(StrTpm)
   If (DGVCap.Rows.Count > 0) Then
    ...
    For Each row As DataGridViewRow In DGVCap.Rows
     Dim idCotizacion As String = UsrTPM
     Dim cantidad As Integer = (Convert.ToInt32(row.Cells(0).Value))
     Dim articulo As String = (Convert.ToString(row.Cells(1).Value))
     'No entiendo aca el orden de las columnas, aca deberian ser diferentes
     'Dim nArticulo As String = (Convert.ToString(row.Cells(4).Value))
     'Dim precio As Double = (Convert.ToDouble(row.Cells(2).Value))

     Dim nArticulo As String = (Convert.ToString(row.Cells(2).Value))
     Dim precio As Double = (Convert.ToDouble(row.Cells(3).Value)) 'Col 3 = Precio Esta col es el precio

     Dim descuento As Double = (Convert.ToDouble(row.Cells(4).Value)) 'Col 4 = Descuento
     Dim importe As Double
     If SinDescuento Then
      importe = (precio * cantidad) 'Cambie el valor 6 del importe por la columna 9 del importe original para que siempre calcule en base a ese importe
     Else
      importe = (Convert.ToDouble(row.Cells(9).Value)) 'Cambie el valor 6 del importe por la columna 9 del importe original para que siempre calcule en base a ese importe
     End If
     Dim moneda As String = (Convert.ToString(row.Cells(7).Value))
     Dim lista As Integer = (Convert.ToInt32(row.Cells(8).Value)) 'Col 8 = Lista de Precios

     conexion.Open()
     Dim cadena As String = "INSERT INTO tpm.dbo.temporalDesc (idUsuario,cantidad,articulo,nArticulo,precio,lista,descuento,importe,moneda) values ('" & idCotizacion & "'," & cantidad & ",'" & articulo & "','" & nArticulo & "'," & precio & "," & lista & "," & descuento & "," & importe & ",'" & moneda & "')"
 ...
 Private Sub MostrarDescuentos()
  ...
  Using connection As SqlConnection = New SqlConnection(args)
   command = New SqlCommand("SP_CalcularDescuentosporVolumen2", connection)
   command.CommandType = CommandType.StoredProcedure
   command.Parameters.AddWithValue("@IdUsuario", UsrTPM)
   adapter = New SqlDataAdapter(command)
   dtTable = New DataTable
   Try
    adapter.Fill(dtTable)

    'SECCION PARA COLOCAR EN LAS COLUMNAS LA INFORMACION DEL DESCUENTO
    If dtTable.Rows.Count <> 0 Then
     For Each fila As DataRow In dtTable.Rows
      For Each filaDGC As DataGridViewRow In DGVCap.Rows
       If (filaDGC.Cells(1).Value = fila("Articulo")) Then
        DGVCap(4, filaDGC.Index).Value = fila("descuento") 'Coloco el descuento
        DGVCap(5, filaDGC.Index).Value = fila("importe")
        Exit For
       End If
      Next
     Next
    End If
    'TERMINA SECCION PARA COLOCAR EN LAS COLUMNAS LA INFORMACION DEL DESCUENTO
 ...
 Private Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click
  ...
  lblEspera.Refresh()
  Me.Refresh()
  recuperarCotizacion2()
  lblEspera.Visible = False
  MsgBox("Debido a las actualizaciones de los precios y/o descuentos es sumamente recomendable que confirme la cantidad de articulos de cada renglón", MsgBoxStyle.OkOnly, "Aviso importante!!")
 End Sub

 Private Sub recuperarCotizacion2()
  Dim args As String = conexion_universal.CadenaSQL

  Dim command As SqlCommand
  Dim adapter As SqlDataAdapter
  Dim dtTableDetalles As DataTable
  Dim infCte As DataTable
  Dim dsCotizacion As New DataSet
  Dim dvCliente As New DataView
  Dim numCoti As String
  Using connection As SqlConnection = New SqlConnection(args)

   'Validar que sea numero
   Try
    numCoti = Convert.ToInt32(txtnumCot.Text)
   Catch ex As Exception
    Exit Sub
   End Try

   command = New SqlCommand("RecuperarCotizacion", connection)
   command.CommandType = CommandType.StoredProcedure
   command.Parameters.AddWithValue("@idCotizacion", numCoti)
   adapter = New SqlDataAdapter(command)
   dtTableDetalles = New DataTable
   infCte = New DataTable
   Try
    'adapter.Fill(dtTable)

    adapter.Fill(dsCotizacion, "dsCotizacion")
    infCte = dsCotizacion.Tables(0)

    If (dsCotizacion.Tables(0).Rows.Count <= 0) Then
     MsgBox("La cotización indicada no existe", MsgBoxStyle.OkOnly, "Cotización no registrada")
     txtnumCot.Focus()
     txtnumCot.SelectionStart = txtnumCot.ToString.Length
     txtnumCot.SelectAll()

     Exit Sub
    End If

    For i = 0 To CmbCliente.Items.Count
     CmbCliente.SelectedIndex = i
     If dsCotizacion.Tables(0).Rows(0).Item(0).ToString = CmbCliente.SelectedValue.ToString Then
      Exit For
     End If
    Next
    ...
    TxtComentario.Text = dsCotizacion.Tables(1).Rows(0).Item(0)

    dtTableDetalles = dsCotizacion.Tables(2)

    'SECCION PARA COLOCAR EN LAS COLUMNAS LA INFORMACION DE LA COTIZACION RECUPERADA
    For Each fila As DataRow In dtTableDetalles.Rows
     If YaEstaAcomodado = False Then
      acomodarColumnas()
     End If
     Me.DGVCap.Rows.Add(fila("Cantidad"), 'Col 1
                        fila("Articulo"), 'Col 2
                        fila("nArticulo"),'Col 3
                        fila("precio"),   'Col 4
                        fila("descuento"),'Col 5
                        fila("importe"),  'Col 6
                        fila("linea"),    'Col 7
                        fila("Moneda"),   'Col 8
                        fila("lista"),    'Col 9
                        fila("importe"))  'Col 10
     'Por seguridad revalida los cambios en los precios
     'ValidoRenglon(0)
    Next
    'TERMINA SECCION PARA COLOCAR EN LAS COLUMNAS DE LA COTIZACION RECUPERADA
   ...
   End Try
  End Using
  SinDescuento = True 'Coloco e indico sin descuento para que ralice la operacion correctamente
  CalculaDscuentos()
  SinDescuento = False
  'SumarCantidades()
 End Sub
 ...
 Private Sub Button4_Click(sender As Object, e As EventArgs)
  ...
  For Each row As DataGridViewRow In DGVArt.Rows
   MessageBox.Show(DGVCap.Columns(0).Name)

   MessageBox.Show(DGVArt.Columns(1).Name)

   MessageBox.Show(DGVArt.Columns(2).Name)
   MessageBox.Show(DGVArt.Columns(3).Name)

   MessageBox.Show(DGVArt.Columns(4).Name)
   MessageBox.Show(DGVArt.Columns(5).Name)
   MessageBox.Show(DGVArt.Columns(6).Name)
   MessageBox.Show(DGVArt.Columns(7).Name)
   MessageBox.Show(DGVArt.Columns(8).Name)
 ...
 Private Sub SumarCantidades()
  If (DGVCap.Rows.Count > 0) Then
   Dim suma As Double = 0
   For Each row As DataGridViewRow In DGVCap.Rows
    suma += Format(Convert.ToDouble(row.Cells(5).Value), "#,###,##0.00")
   Next
   txtTotal.Text = suma.ToString("N")
  Else
   txtTotal.Text = "0.00"
  End If
 End Sub
